<template>
    <div class="bigDec">
        ...
        <div @click="handleClick">
            <div class="bigDec__price">
                <div class="bigDec__price--left">
                    <div>
                        <span>￥</span>
                        <!-- 类型：1-团购；2-单品套餐 -->
                        {{ flashSalePrice[0] }}
                        <text v-if="flashSalePrice[1]">.{{ flashSalePrice[1] }}</text>
                    </div>
                    <div>市场价:{{ item.type === 1 ? item.marketPrice : item.specialPrice || item.marketPrice }}</div>
                </div>
                <div class="bigDec__price--right" v-if="!!item.surplusAmount">仅剩:{{ item.surplusAmount }}份</div>
            </div>
            <div class="bigDec__title oneLine">
                {{ item.title }}
            </div>
            <div class="bigDec__tradeName">
                <image src="https://oss.dcqcjlb.com/51che_java_dev/20231113/file_1699867734794.png"></image>
                {{ item.baseName }}
            </div>
        </div>
    </div>
...
export default {
    ...
    computed: {
        ...
        flashSalePrice() {
            let val = "";
            if (this.item.type === 1) {
                val = this.item.specialPrice || this.item.flashSalePrice;
            } else {
                val = this.item.flashSalePrice;
            }
