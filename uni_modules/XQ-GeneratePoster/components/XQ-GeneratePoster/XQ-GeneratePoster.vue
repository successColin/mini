				...
				if (!this.ctx) {
					...
					Promise.all([headImg, code, cover, bgUrl]).then(result => {
						...
						let canvasWidthPx = 640 * this.ratio,
							...
							codeurl_width = 130, //绘制的二维码宽度
							codeurl_heigth = 130, //绘制的二维码高度
							codeurl_x = 520, //绘制的二维码在画布上的位置
							codeurl_y = 740, //绘制的二维码在画布上的位置
							coverurl_width = 600, //绘制的封面宽度
							coverurl_heigth = 600, //绘制的封面高度
							coverurl_x = 45, //绘制的封面在画布上的位置
							coverurl_y = 30; //绘制的封面在画布上的位置
						//背景
						if (option.bgUrl) {
							...
							ctx.translate(0, 0);
							//绘制圆角矩形的各个边
							this.drawRoundRectPath(ctx, 345, 520, 14);
						...
						ctx.fillText('活动截止时间：' + option.signUpEndTimeStr, 45, 770); // 绘制孩子百分比
						//人民币符号
						ctx.font = '24px sans-serif';
						ctx.setFillStyle('#d91b1b'); // 文字颜色
						ctx.fillText('￥', 45, 825); // 绘制孩子百分比
						//抢购价
						ctx.font = 'normal bold 36px sans-serif';
						ctx.setFillStyle('#d91b1b'); // 文字颜色
						ctx.fillText(option.flashSalePrice, 70, 825); // 绘制孩子百分比
						...
						ctx.draw(false, () => {
							...
							uni.canvasToTempFilePath({
									...
									success: res => {
										...
										this.$emit('sendimg', this.canvasToTempFilePath)
										uni.showToast({
											title: '绘制成功'
										});
