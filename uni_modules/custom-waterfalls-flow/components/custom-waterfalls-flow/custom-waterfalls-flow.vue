<template>
	<view class="waterfalls-flow">
		<view v-for="(item,index) in data.column" :key="index" class="waterfalls-flow-column"
			:id="`waterfalls_flow_column_${index+1}`" :msg="msg" :style="{'width':w,'margin-left':index==0?0:m}" style="position: relative;">
			<view :class="['column-value',{'column-value-show':item2.o}]" v-for="(item2,index2) in columnValue(index)"
				...
				<image @click.stop="imageClick(item2)" style="width: 66rpx;height: 66rpx;" src="https://oss.dcqcjlb.com/51che_java_dev/20230815/file_1692088088992.png"></image>
									</view>
				<image 
