<template>
    <view class="waterfalls-flow">
        <view v-for="(item, index) in data.column" :key="index"
            :class="['waterfalls-flow-column', `waterfalls-flow-column-${index + 1}`]"
            :id="`waterfalls_flow_column_${index + 1}`" :msg="msg"
            :style="{ 'width': w, 'margin-left': index == 0 ? 0 : m }" style="position: relative;">
            <view :class="['column-value', { 'column-value-show': item2.o }, { 'column-value-position': item2.pa }]"
                v-for="(item2, index2) in columnValue(index)" :key="index2" :style="[s1]" @click.stop="wapperClick(item2)">
                ...
                <view class="watch-icon" v-if="isshowicon && item2.video">
                    <image @click.stop="imageClick(item2)" style="width: 66rpx;height: 66rpx;"
                        src="https://oss.dcqcjlb.com/51che_java_dev/20230815/file_1692088088992.png"></image>
                </view>

                <img v-if="item2.videoUrl" src="https://oss.dcqcjlb.com/51che_java_dev/20231114/file_1699949044520.png"
                    class="play" />
