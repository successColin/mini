			...
			>
				...
				>
					...
					<image
						...
						:src="getSource(item)"
						:mode="imgMode"
						@tap="clickHandler(index,1)"
						:style="{
							height: $u.addUnit(height),
							borderRadius: $u.addUnit(radius)
						}"
					></image>
					<view v-if="getItemType(item) === 'video'" @tap="clickHandler(index,2)" style="position: relative;width:100%;">
						<image
						class="u-swiper__wrapper__item__wrapper__image"
						:src="getPoster(item)"
						:mode="imgMode"
						...
						<image src="https://oss.dcqcjlb.com/51che_java_dev/20230519/file_1684478860207.png" style="position: absolute;top: calc(50% - 60rpx);left: calc(50% - 60rpx);width: 120rpx;height: 120rpx;"></image>
					</view>
					
		<!-- 			<view v-if="getItemType(item) === 'video'" style="position: relative;width: 100%;height: 100%;" @tap="clickHandler(index,2)">
						<video
						...
						:style="{
							height: $u.addUnit(height),width: '100%'
						}"
						:controls="false"
						@tap="clickHandler(index,2)">
						</video>
						<view style="position: absolute;width: 100%;height: 100%;opacity: 0;"></view>
					</view> -->
					<text
...
<script>
	 ...
	 * @example	<u-swiper :list="list4" keyName="url" ></u-swiper>
	 */
	export default {
		name: 'u-swiper',
