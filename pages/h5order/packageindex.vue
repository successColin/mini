	...
	<view class="page">
		...
		<view v-if="index==0">
			<view class="card">
				...
				<view class="mt10">
					<text class="mainRed fwb size28"><text>￥</text><text>{{dataList.flashSalePrice}}</text></text>
					<text
						class="txtLighGray txtls ml10 size24"><text>￥</text><text>{{dataList.marketPrice}}</text></text>
				</view>
			</view>
			<view class="card">
				...
				<view class="mt20">
					...
					<view class="size28 mt10">到店核销<text v-if="dataList.holidaysIsUse==1" class="ml10">节假日通用</text>
					</view>
				</view>
			</view>
			<view class="card">
				...
				<view v-for="(item,index) in dataList.mealLists" :key="index">
					<view class="u-flex jsb mt20 size24">
						...
						<view>
							<text class="txtLighGray">{{item.num}}</text>
