	...
	<view class="page">
		...
		<view v-if="index==0">
			...
			<view class="card">
				...
				<view class="mt20">
					...
					<view class="size28 mt10">到店核销<text v-if="dataList.holidaysIsUse==1" class="ml10">节假日通用</text></view>
				</view>
			</view>
			<view class="card">
				<view class="size32 fwb">
		...
		<view v-else-if="index==1">
			<view class="u-flex jsb margin23">
				...
				<view class="card-detail">
					<view class="size32 mt30">收入合计</view>
			...
			<view class="card-order" v-for="(item,index) in verificationlist.orderDetail" :key="index">
				<view class="u-flex jsb alc">
					<view class="txtLighGray" :class="{'redcolor':item.status==1}">
						<text class="size28 fwb">¥{{item.paidMoney}}</text>
						<text class="size24 ml10">套餐收入</text>
					</view>
