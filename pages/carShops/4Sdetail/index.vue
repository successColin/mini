	...
	<view class="page">
		<view v-if="isshowpage">
			<view class="top-detail">
				<view class="u-flex">
					<view>
						<image mode="heightFix" style="width: 70rpx;height: 43rpx;" :src="detail.brandLogo"></image>
					</view>
					<view class="shop-name">
						{{ detail.storeAbbreviation }}
					</view>
				</view>
				<view v-if="detail.doorheadPhotos.length == 1" style="margin-top: 22rpx;">
					<image mode="aspectFill" style="width: 650rpx;height: 210rpx;border-radius: 10rpx;"
						:src="detail.doorheadPhotos[0]">
					</image>
				</view>
				<view v-else-if='detail.doorheadPhotos.length == 3' style="margin-top: 22rpx;">
					<image mode="aspectFill" style="width: 210rpx;height: 210rpx;border-radius: 10rpx;"
						:src="detail.doorheadPhotos[0]">
					</image>
					<image mode="aspectFill"
						style="width: 210rpx;height: 210rpx;border-radius: 10rpx;padding-left: 10rpx;"
				...
				<view class="top-contact flex alc mt30">
					<view @click="OnLocation" class="u-flex">
						<view>
							<image style="width: 24rpx;height: 30rpx;" src="@/pages/carShops/static/image/dingwei.png">
							</image>
						</view>
					...
					<view v-if="detail.mobile" @click="Oncall" class="u-flex ml20 mr20">
						<view>
							<image style="width: 26rpx;height: 25rpx;" src="@/pages/carShops/static/image/dianhua.png">
							</image>
						</view>
						<view style="font-size: 24rpx;margin-left: 12rpx;">
							{{ detail.mobile }}
			...
			<u-tabs :inactiveStyle="{
					...
					fontSize: '28rpx',
					transform: 'scale(1)'
				}" :activeStyle="{ color: '#303133', fontWeight: 'bold', fontSize: '32rpx', transform: 'scale(1.05)'}"
				lineColor='#D91B1B' lineWidth="16" :list="tag" @click="clicktag" itemStyle="height:88rpx;width:160rpx"
				:is-scroll="true" :current='tabsindex'>
			</u-tabs>

			<!-- 轮播 -->
			<view class="carousel">
				<carousel type="2" :shopId='id' ref='carousel' :topClass="['activity-img']" height="260rpx" urlkey="url"
					:dataType="2" :systemCode="systemCode" :indicatorType="3" v-model="bannerList"></carousel>
			</view>
			<view v-if="dataTag==6">
				<view class="usercarassess">
					<u--form labelPosition="left" :model="cardata" :rules="rules" ref="uForm" labelWidth='80'
						:labelStyle="{fontWeight:'bold'}">
						<u-form-item label="品牌车型" prop="brandVehicle" borderBottom ref="item1">
							<u--input inputAlign='right' placeholder="请输入品牌如：奥迪A4L" border="none"
								v-model="cardata.brandVehicle"></u--input>
						</u-form-item>
						<u-form-item label="车辆年份" prop="carYear" borderBottom ref="item1">
							<u--input type='number' inputAlign='right' placeholder="请输入年份如：2023" border="none"
								v-model="cardata.carYear"></u--input>
						</u-form-item>
						<u-form-item label="行驶里程" prop="mileage" borderBottom ref="item1">
							<u--input type='number' inputAlign='right' placeholder="请输入行驶里程如：6000" border="none"
								v-model="cardata.mileage"></u--input>
						</u-form-item>
						<u-form-item label="姓名" prop="customerName" borderBottom ref="item1">
							<u--input inputAlign='right' placeholder="请输入姓名" border="none"
								v-model="cardata.customerName"></u--input>
						</u-form-item>
						<u-form-item label="手机号" prop="customerMobile"  ref="item1">
							<u--input type='number' inputAlign='right' placeholder="请输入车主手机号" border="none"
								v-model="cardata.customerMobile"></u--input>
						</u-form-item>
					</u--form>
				</view>
			...
			<!-- 筛选框 -->
			<view v-if="(dataTag == 1 || dataTag == 2) && !showuloadingicon" class="select"
				:class="{ 'sticky-box': !isSticky }" style="padding-bottom: 15rpx;">
				<view class="u-flex-al">
					<view class="list-tag" @click="openSearch(3)" style="justify-content: flex-end">
					...
					<view class="list-tag" @click="openSearch(2)" style="justify-content: flex-end">
						<view class="text ellipsis">
					...
					<view class="list-tag" @click="openSearch(1)" style="justify-content: flex-end">
						<view class="text ellipsis">
					...
					<view class="list-tag" @click="openSearch(4)" style="justify-content: flex-end">
						<view class="text ellipsis">
			...
			<view v-if="dataTag == 1 && !showuloadingicon">
				...
				<view v-else style="margin-top: 120rpx;text-align: center;">
					<image style="width: 250rpx;height: 250rpx;" src="../static/image/zanwucheliang.png"></image>
				</view>
			</view>
			<view v-if="dataTag == 2 && !showuloadingicon">
				<view v-if="dataList.length != 0">
					<view @click="OnCarDetail(2, item)" v-for="(item, index) in dataList" :key="index"
						...
						<view class="newCarR">
							...
							<view style="margin-left: -12rpx;margin-top: 16rpx;" class="u-flex">
								<view class="tag-style" v-for="(itemtag, indextag) in item.biaoqian[0]" :key="indextag">
									<text class="tag-text">{{ itemtag }}</text>
								</view>
							</view>

							<view class="size24" style="color: #999999;margin-top: 16rpx;margin-bottom: 16rpx;">
							...
							<view class="u-flex" style="align-items: baseline;">
								<view style="font-size: 32rpx;font-weight: bold;color: #D91B1B;">
									{{ (item.price * 0.0001).toFixed(2) }}<text
										style="font-size: 24rpx;margin-left: 6rpx;">万</text>
								</view>
			...
			<view v-if="dataTag == 3 && !showuloadingicon">
				...
				<view class="tagmaintenance-card" style="margin: 25rpx;margin-bottom: 10rpx">
					<view class="tac" @click="Onpushtag(1)">
						<image src="../static/image/wodeaiche.svg" style="width: 66rpx;height: 66rpx;"></image>
						<view class="mt15">
							我的爱车
						</view>
					</view>
					<view class="tac" @click="Onpushtag(2)">
						<image src="../static/image/weibaoyuyue.svg" style="width: 66rpx;height: 66rpx;"></image>
						<view class="mt15">
							维保预约
						</view>
					</view>
					<view class="tac" @click="Onpushtag(3)">
						<image v-if="tagmaintenance!='3'" src="../static/image/baoyangzhuanghuang.svg"
							style="width: 66rpx;height: 66rpx;"></image>
						<image v-else-if="tagmaintenance=='3'" src="../static/image/xuanzhongbaoyangzhuanghuang.svg"
							style="width: 66rpx;height: 66rpx;"></image>
					...
					<view class="tac" @click="Onpushtag(4)">
						<image v-if="tagmaintenance!='4'" src="../static/image/guzhangzixun.svg"
							style="width: 66rpx;height: 66rpx;"></image>
						<image v-else-if="tagmaintenance=='4'" src="../static/image/xuanzhongguzhangzixun.svg"
							style="width: 66rpx;height: 66rpx;"></image>
				...
				<view v-if="tagmaintenance=='3'">
					...
					<view style="margin-left: 25rpx;" @click="OnInsurance ">
						<image style="width: 700rpx;height: 168rpx;"
							src="https://oss.dcqcjlb.com/51che_java_dev/20230918/file_1695034534918.png">
						</image>
					</view>
					<view style="margin:25rpx;margin-top: 10rpx;">
						<u-search :showAction='false' placeholder="搜索保养套餐" v-model="packagetitle"
							@search='getList'></u-search>
					</view>
					<view class="upkeepList flex" v-if="dataList.maintainMealListVOS.length != 0">
						<view @click="OnPushDetail(item)" v-for="(item, index) in dataList.maintainMealListVOS"
							:key="index" class="upkeepItem mb10">
							<view style="position: relative;">
								<image :src="item.cover_image" class="upkeepImg" mode="aspectFill"></image>
							...
							<view class="upkeepCont">
								...
								<view class="flex priceLine">
									...
									<view class="huiPrice">{{ item.bankCardPrice ? item.bankCardPrice : item.price }}
									</view>
									<view class="txtLighGray size24 ml10 txtls">原价{{ item.market_price }}元</view>
								</view>
								<view class="flex alc jsb">
									<view class="txtLighGray size24">{{ detail.storeAbbreviation }}</view>
									<view class="goShop" v-if="iscommission == 0">购买</view>
									<view @click.stop="Onjoinshop(item, index, 4)" class="goShop"
										v-else-if="iscommission == 1" :class="{ 'Asreminder': item.isSelection == 1 }">
										加小店
									</view>
								</view>
							</view>
					...
					<view v-else style="margin-top: 120rpx;text-align: center;">
						<image style="width: 250rpx;height: 250rpx;" src="../static/image/zanwubaoyang.png"></image>
					</view>
					<view v-if='iscommission == 0' class="contact"
						:style="{ paddingBottom: safeHeight == 0 ? 0 : '10rpx' }">
						<view @click="Oncontact" class="btn">
							联系客户经理
						</view>
					</view>
			...
			<view v-if="dataTag == 4 && !showuloadingicon">
				<view v-if="iscommission == 0" class="activity-tab">
					<view class="tab-item" :class="{ 'ctab-item': activitytag == 1 }" @click="Onactivity(1)">
						户内嗨乐
					</view>
					<view class="tab-item" :class="{ 'ctab-item': activitytag == 3 }" @click="Onactivity(3)">
						户外畅游
					</view>
					<view class="tab-item" :class="{ 'ctab-item': activitytag == 4 }" @click="Onactivity(4)">
						优惠资讯
					</view>
					<view class="tab-item" :class="{ 'ctab-item': activitytag == 2 }" @click="Onactivity(2)">
						精彩回放
					</view>
				</view>
				<view v-if="activitytag==4" class="consulting-service">
					<view v-for="(item, index) in brandNewList" :key="index" class="card" @click="Onbrand(item)">
						<view>
						...
						<view style="margin-left: 21rpx;position: relative;">
							...
							<view class="u-flex alc"
								style="font-size: 24rpx;color: #999999;position: absolute;bottom: 0rpx;width: 514rpx;justify-content: space-between;position: absolute;bottom: 0rpx;">
							<!-- 	<view>{{ item.shopName }}
								</view> -->
								<view>{{ item.createdAt }}
								</view>
								<view class="typemsg-btn">{{ item.typeMsg }}
								</view>
							</view>
						</view>
					</view>
				...
				<view v-if="activitytag==1||activitytag==3" style="margin: 25rpx;">
					<view v-if="dataList.length != 0">
						<view v-for="(item, index) in dataList" :key="index" style="margin-top: 47rpx;"
							...
							<view class="coverPictureactivity"
								...
								<view
									...
									<text v-if="item.activitySignUpStatus == 2"
										style="margin-right: 14rpx;">报名截止时间</text>
									<text style="margin-right: 14rpx;"
										v-if="item.activitySignUpStatus == 1">报名开启时间</text>{{
										item.activityBeginTime }}
								</view>

								<view v-if="item.activitySignUpStatus == 1 || item.activitySignUpStatus == 2"
									class="backimg">
								...
								<view style="position: absolute;left: 260rpx;top:110rpx ;"
									...
									<image src="@/static/image/huodongyugao.png" style="width: 180rpx;height: 180rpx;">
									</image>
								</view>
								<view style="position: absolute;left: 260rpx;top:110rpx ;"
									v-if="item.activitySignUpStatus == 3 || item.activitySignUpStatus == 5 || item.activitySignUpStatus == 4">
									<image src="@/static/image/jieshu.png" style="width: 180rpx;height: 180rpx;">
									</image>
								</view>
				...
				<view v-if="activitytag==2">
					<view v-if="dataList.length != 0"
						style="display: flex;flex-wrap: wrap;justify-content: space-between;margin: 32rpx 20rpx;">
						<view v-for="(item, index) in dataList" :key="index" style="margin-top: 40rpx;"
							@click="OnPushreviewDetail(item)" :class="{ 'margintop0': index == 0 || index == 1 }">
							<view v-if="item.coverPicture" style="position: relative;">
								<image :mode="mode" :src="item.coverPicture" class="coverPicture">
								</image>
							...
							<view v-else>
								<image :mode="mode" :src="item.img" class="coverPicture"></image>
							</view>
					...
					<view v-else-if='activitytag==2' style="margin-top: 120rpx;text-align: center;">
						<image style="width: 250rpx;height: 250rpx;"
			...
			<!-- 	<view v-if="dataTag == 5 && !showuloadingicon">
				<view v-if="dataList.length != 0" style="margin-top: 20rpx;padding: 0 25rpx;">
						...
						<view @click="lookCoupon(item)">
							<view class="flex jsb HuiNum">
								<view class="flex mt10">
									<text class="size32 fwb leftOneLine oneLine">{{ item.name }}</text>
								</view>
				...
				<view v-else style="margin-top: 20rpx;text-align: center;">
					<image style="width: 250rpx;height: 250rpx;" src="../static/image/zanwuyouhui.png"></image>
				</view>
				<view v-if="brandNewList.length > 0" class="consulting-service">
					<view class="title">
						品牌资讯
					</view>
					<view v-for="(item, index) in brandNewList" :key="index" class="card" @click="Onbrand(item)">
			...
			<view v-if="dataTag == 6 && !showuloadingicon" class="w700 recommend">
				<custom-waterfalls-flow imageKey="coverPicture" ref="waterfallsFlowRef" :value="dataList"
					...
					<view v-for="(item, index) in dataList" :key="index" class="recommend-item" slot="slot{{index}}">
						<view class="recommend-item-bottom">
							<view class="recommend-item-bottom-title">{{ item.activityTitle || item.articleTitle ||
								item.productTitle
							}}
							</view>
							<view class="flex alc jsb">
								<view class="flex alc" style="padding-left: 10rpx;">
									...
									<view class="recommend-item-bottom-name">{{ item.nickname }}</view>
								</view>
								<view class="recommend-item-bottom-check">{{ item.viewBaseNum }}人看过</view>
							</view>
						</view>
					</view>
				</custom-waterfalls-flow>
			</view>
			<view v-if="isLoadMore && dataList.length != 0&&tagmaintenance!='4'" class="size24 tac txtLighGray"
			...
			<view class="top-search">
				<view class="select" style="margin-top: 0;">
					<view class="u-flex-al">
						<view class="u-flex-al" @click="openSearch(3)"
							style="width: 171rpx;justify-content: flex-end;margin-left: 15rpx;">
						...
						<view class="u-flex-al" @click="openSearch(2)"
							style="width: 144rpx;justify-content: flex-end;margin-left: 15rpx;">
						...
						<view class="u-flex-al" @click="openSearch(1)" style="width: 160rpx;justify-content: flex-end;">
						...
						<view class="u-flex-al" @click="openSearch(4)" style="width: 160rpx;justify-content: flex-end;">
							<view class="text ellipsis" :class="{ 'text-bold': searchType === 4 }"
								style="max-width: 120rpx;">
								{{ searchField.autofinance ? searchField.autofinance : '汽车金融' }}
							</view>
							<view style="margin-left: 11rpx;">
				...
				<view class="top-search-btn-view">
					<template v-if="searchType === 1">
						...
						<view v-for="(item, index) in carPriceList" :key="index" class="top-search-btn"
					...
					<template v-if="searchType === 3">
						...
						<view v-for="(item, index) in energyTypeList" :key="index" class="top-search-btn"
							@click="searchItem(item)"
					...
					<template v-if="searchType === 4">
						<view :key="-1" class="top-search-btn" @click="searchItem({ id: '', name: '全部' })"
							:style="{ color: searchField.autoFinance == '' ? '#D91B1B' : '#222222', background: searchField.autoFinance == '' ? '#FFF1F1' : '#F8F8F8' }">
							<text class="top-search-btn-text ellipsis">全部</text>
						</view>
						<view v-for="(item, index) in autofinanceList" :key="index" class="top-search-btn"
							@click="searchItem(item)"
							:style="{ color: searchField.autoFinance == item.id ? '#D91B1B' : '#222222', background: searchField.autoFinance == item.id ? '#FFF1F1' : '#F8F8F8' }">
							<text class="top-search-btn-text ellipsis">{{ item.name }}</text>
						</view>
					</template>
				</view>
		...
		<u-popup :safeAreaInsetBottom='false' :show="showcontact" @close="showcontact = false" mode="bottom">
			<view class="pop-title">联系客户经理</view>
			<view class="pop-Tips">
				<view>
					<image src="../static/image/shouzhi.png" style="width: 28rpx;height: 34rpx;"></image>
				</view>
			...
			<view v-for="(item, index) in dataList.customerManagerListVOS" :key="index"
				...
				<view class="manger-list">
					...
					<view @click="OnManger(item)" class="pop-choose">
						选择
					</view>
				</view>
			</view>
		...
		<u-popup :safeAreaInsetBottom='false' :show="showcode" @close="showcode = false" mode="center">
			<view style="width: 700rpx;height: 700rpx;">
				<view style="text-align: center;margin-top: 50rpx;">
					<image @click="OnpreviewImage" :show-menu-by-longpress="true" style="width: 440rpx;height: 444rpx;"
						:src="dataList.code"></image>
				</view>
	...
	export default {
		...
		onLoad(option) {
			...
			if (option.scene) {
				...
				this.$newrequest.post('/coc-system/api/v1/system/customer/manager/detail', {
		...
		methods: {
			Onadd() {
				...
				this.$refs.uForm.validate().then(res => {
					this.$request.post("/coc-active/api/v1/four_s/clues/used/car/evaluation/add",this.cardata).then(res=>{
						if(res.code==200){
							uni.$u.toast('操作成功')
							this.cardata={
								brandVehicle: '',
			...
			Onpushtag(index) {
				if (index == 1) {
					...
					uni.navigateTo({
						url: "/pages/upkeep/bookingForm?shopId=" + this.id + '&brandId=' + this.detail.brandId
					});
				} else if (index == 3) {
			...
			OnManger(item) {
				this.$request.post("/coc-active/api/v1/four_s/customer_manager/bind", {
					id: item.id
				}).then(res => {
					if (res.code == 200) {
			...
			getDetail() {
				...
				this.$newrequest.get("/coc-active/api/v1/four_s/shopDetails/" + this.id).then(res => {
					...
					this.$refs.carousel.getCarouselListshopid('applets_store_car_buying', option.id)
				}).catch(err => {
			...
			OnPushDetail(item) {
				uni.navigateTo({
					url: '/pages/carShops/upkeep/detail?id=' + item.id + '&customerManagerId=' + this
						.customerManagerId + '&iscommission=' + this.iscommission
				})
			...
			getmaintainmeal() {
				this.$newrequest.post('/coc-active/api/v1/four_s/maintain_meal/list', {
					shopId: this.id,
				...
				this.$newrequest.post("/coc-active/api/v2/homePage/getActivitysV2", {
					...
					if (this.iscommission == 1) {
						...
						res.data.homePageActivitysVOIPage.records.filter(s => {
							if (s.activitySignUpStatus == 1 || s.activitySignUpStatus == 2) {
								res.data.homePageActivitysVOIPage.newrecords.push(s)
							}
						})
					} else {
						res.data.homePageActivitysVOIPage.newrecords = res.data.homePageActivitysVOIPage.records
					}
					this.showuloadingicon = false
			...
			getReview() {
				this.$newrequest.post("/coc-active/api/v2/homePage/wonderfulReview", {
					categoryId: 0,
					current: this.current,
					shopTypeId: 0,
					size: 20,
					...
					res.data.marvellousVOIPage.records.filter(s => {
						if (s.videos) {
							s.coverPicture = s.videos + '?x-oss-process=video/snapshot,t_1000,m_fast'
						}
					})
			...
			getusedCar() {
				this.$newrequest.post("/coc-active/api/v1/four_s/usedCar", {
			...
			getcenter() {
				this.$newrequest.post("/coc-active/api/v1/couponNew/shop/ticket/center", {
					current: this.current,
					shopId: this.id,
					size: 99
				}).then(res => {
