<template>
  <view class="page">
    <view class="flex alc mb10 bgfff  contentBox">
      <text>联系方式： </text>
      <input type="text" v-model="payData.uMobile"  @change="changeTel" style="width:490rpx">
    </view>
    <view class="orderHui contentBox bgfff">
      ...
      <view class="flex jsb alc quanCheck" style="padding-bottom:36rpx;" v-if="orderData.couponUserNews.length == 0">
        <view class="huiL">优惠券</view>
        <view class="flex alc">
          <text class="txtLighGray">暂无可用优惠券</text>
        </view>
      </view>
      <view class="flex jsb alc quanCheck" style="padding-bottom:36rpx;" @click="checkQuan" v-else>
        <view class="huiL">优惠券</view>
        <view class="flex alc">
          <text class="mainRed ">{{ choosecoupon.couponUserNewId ? choosecoupon.couponName : '请选择优惠券' }}</text>
    ...
    <view class="mt10 mainRed size24" style="text-align:right;" v-show="minMoney">当前优惠券金额大于等于订单金额，需支付0.01元才可提交</view>
      ...
      if (data.data) {
        ...
        this.orderData.couponUserNews.filter(res => {
          if (res.couponUserNewId == data.data) {
            ...
            this.totalprice = (this.totalprice - this.payData.couponMoney).toFixed(2)
          }
        })
  ...
  methods: {
    ...
    addNum() {
      ...
      if (this.choosecoupon.discountMoney) {
        this.totalprice = this.payData.maintainNum * this.orderData.price - this.choosecoupon.discountMoney
      } else {
        this.totalprice = this.payData.maintainNum * this.orderData.price
      }
      if (this.totalprice <= 0) {
    ...
    // 获取订单详情
    getOrder(id) {
      this.$newrequest.post("/coc-active/api/v2/maintain_meal/buyNow", { maintainMealId: id }).then(res => {
    ...
    OnPay() {
      ...
      this.$request.post("/coc-active/api/v2/maintain_meal/payment", that.payData).then(res => {
        if (res.code == 200) {
