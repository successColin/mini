  ...
  <view class="page">
    ...
    <view @touchstart="touchStart" @touchend="touchEnd">
      <view v-for="item in dataList" :key="item.id" class="card-list">
        <big-dec :item="item" v-if="bigIndex === 0"></big-dec>
        <shop-menu :item="item" v-else></shop-menu>
      </view>
...
export default {
  ...
  onLoad(option) {
    ...
    if (homeMenuId) {
      this.searchField.homeMenuId = homeMenuId * 1;
    }
    // 大分类下的子分类
  ...
  onShareAppMessage(res) {
    ...
    return {
      title: title,
      imageUrl: imageUrl,
      path: "/pages/gourmetGroggery/index?enter=117",
      success(res) {
          uni.showToast({
              title: "分享成功",
          });
  ...
  methods: {
    getshareimg() {
      this.$newrequest.get("/coc-system/api/v1/system/dictionaries/pageLayout/details/37").then((res) => {
    ...
    touchEnd(e) {
      ...
      if (deltaX > 0) {
        if (this.bigIndex === 0) return;
        this.bigIndex -= 1;
      } else {
        if (this.bigIndex === this.bigType.length - 1) return;
        this.bigIndex += 1;
      }
      this.getStatus(this.bigType[this.bigIndex], this.bigIndex);
    },
    // 点击最大的
    getStatus(item, i) {
      this.bigIndex = i;
