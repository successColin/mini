		...
		<view v-if="isshowpage">
			<view class="card" @click="OnPushDetail">
				...
				<view class="u-flex">
					...
					<view style="margin-left: 23rpx;">
						...
						<view class="size24" style="margin-top: 14rpx;text-align: right;">
							<view>
			...
			<view class="card">
				<view class="u-flex jsb">
					...
					<view class="txtLighGray size24 txtls">￥{{dataList.mgroupPrice==0?dataList.marketPrice:dataList.mgroupPrice}}</view>
				</view>
				<u-divider :text="null"></u-divider>
				<view class="u-flex jsb">
			...
			<view v-if="dataList.status==0" style="margin: 0 25rpx 20rpx 25rpx;">
				<view class="On-pay">
			...
			<view class="card-goumai">
				<ul class="ul-item">
					...
					<li v-if="dataList.nonRefund==1" class="li-item"><text
							class="txtDarkGray">本商品为限时限量优惠套餐，请您在订单使用时间内及时消费使用，一经购买不支持退款</text></li>

					<li v-if="dataList.remark" class="li-item"><text class="txtDarkGray">特殊说明：{{dataList.remark}}</text>
					</li>
					<li class="li-item">
							<text class="txtLighGray">购买后需到店核销使用，请知悉，谢谢</text>
					</li>
				</ul>
			</view>
			<view class="card">
				...
				<view v-for="(item,index) in dataList.merchantProductListVOS" :key="index">
...
<script>
	...
	export default {
		...
		methods: {
			...
			OnPushDetail() {
				uni.navigateTo({
					url: '/pages/grouppurchase/packagedetail?id=' + this.dataList.groupPurchaseProductId
				})
			},
			Ondrawback() {
				let _this = this
				uni.navigateTo({
					url: '/pages/grouppurchase/packageorder/drawbackindex?id=' + this.id,
			...
			OnPay() {
				this.$request.post("/coc-active/api/v1/merchant_meal_order/payment", {
					merchantMealId: this.dataList.groupPurchaseSetMealId,
					merchantMealNum: this.dataList.groupPurchaseSetMealNum,
			...
			getList() {
				...
				this.$request.post('/coc-active/api/v1/merchant_meal_order/orderDetails', {
					orderId: this.id
				}).then(res => {
