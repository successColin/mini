<template>
	<view class="page">
		<view v-for="(item,index) in dataList" :key="index" class="card" @click="OnPushDetail(item)">
			<view class="u-flex jsb">
				<view class="u-flex">
					<view>
						<image style="width: 32rpx;height: 32rpx;"
							src="https://oss.dcqcjlb.com/51che_java_dev/20231011/file_1697008307662.png"></image>
					</view>
					<view class="ml10 size24 cblack">{{item.vehicleName}}</view>
					<view class="ml10 size24 cblack">{{item.plate}}</view>
				</view>
				<view class="mainRed size24" :class="{'txtLighGray':item.status==6}">{{item.statusMsg}}</view>
			</view>
			<view class="mt10 u-flex">
				<view>
					<image style="width: 32rpx;height: 32rpx;"
						src="https://oss.dcqcjlb.com/51che_java_dev/20231011/file_1697008644592.png"></image>
				</view>
			...
			<view class="mt10 u-flex jsb alc">
				<view class="u-flex">
					<view>
						<image style="width: 32rpx;height: 32rpx;"
							src="https://oss.dcqcjlb.com/51che_java_dev/20231011/file_1697009875490.png"></image>
					</view>
					<view class="ml10 size24 cblack">{{item.baseName}}</view>
				</view>
				<view class="closeorder" v-if="item.status==1" @click.stop="Oncloseorder(item)">
					取消
				</view>
			</view>
		</view>
		<u-popup :safeAreaInsetBottom='false' :show="showpop" @close="showpop=false" mode="center">
			<view class="tac" style="width: 400rpx;padding: 60rpx 130rpx 60rpx 130rpx;">
...
<script>
	export default {
		...
		methods: {
			...
			getList(){
				this.$request.post("/coc-active/api/v1/maintenance_appointment/list",{
