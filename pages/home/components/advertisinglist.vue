<template>
  <about left="达人推荐" right="去逛逛" @click="toList">
        ...
        >
          <view v-if="v.tag !== 16">
            ...
            <view class="flex alc jsb item-avatar">
              ...
              <view
                v-if="v.advSpaceType === 1 || v.advSpaceType === 2"
                class="item-check"
                >{{ v.viewNum }}人看过
              </view>
          ...
          success: (res) => {
            ...
            if (res.status !== 3) {
              ...
              this.rows.list.unshift({
                ...
                pa: true,
                coverPicture:
                  'https://oss.dcqcjlb.com/51che_java_dev/20231026/file_1698305656636.png',
              });
              // this.$refs.waterfallsFlowRef.refresh();
              resolve();
            } else {
    ...
    async getList(more) {
      ...
      records.forEach((v) => {
        if (v.imgsMsg) {
          ...
          v.coverPicture =
            v.videoUrl + '?x-oss-process=video/snapshot,t_1000,m_fast';
        }
        list.push(v);
      });
      console.log(list);
