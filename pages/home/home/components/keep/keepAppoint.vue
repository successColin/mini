<template>
	<view class="page">
		<view class="btn_img_div flex jsb">
			<image class="btn_img" src="https://oss.dcqcjlb.com/51che_java_dev/20240109/file_1704787634287.png"
				@click="jump(2)">
			</image>
			<image class="btn_img" src="https://oss.dcqcjlb.com/51che_java_dev/20240109/file_1704787651024.png"
				@click="jump(1)">
			</image>
		</view>
		<image class="process_img" src="https://oss.dcqcjlb.com/51che_java_dev/20240109/file_1704786821008.png"></image>
		<!-- 没有爱车时 -->
		...
		<view class="card">
			...
			<view class="u-flex jsb" @click="onFoursShop">
				<view class="size24 txtLighGray">
					{{ formData.shopName ? formData.shopName : '请选择爱车4S店' }}
				</view>
				<view>
		...
		<!-- 选购保养套餐 -->
		<view class="card" v-else-if="orderlist.length == 0 && meallist.length" @touchstart="mealTouchstart"
			@touchend="mealTouchend">
				...
				<view class="u-flex">
					<view v-for="item in meallist" :key="item.id" class="mr20" @click="OnPushDetail(item)">
						...
						<view class="u-flex mt10 alc">
							...
							<view class="txtLighGray txtls"
								style="font-size: 16rpx;margin-left: 4rpx;margin-top: 2rpx;">
...
<script>
	...
	export default {
		...
		created() {
			...
			uni.$on('car_service_show', () => {
				this.init()
			})
			uni.$on('carService_selshop', value => {
				this.OnFoursData(value)
			})
		},
		destroyed() {
			uni.$off('refresh')
			uni.$off('car_service_show')
			uni.$off('carService_selshop')
		},
		methods: {
			mealTouchstart() {
			...
			togo() {
				...
				this.$request.post("/coc-active/api/v1/maintenance_appointment/save", {
					...
					if (res.code == 200) {
						uni.redirectTo({
							url: '/pages/upkeep/reservationend?shopId=' + this.formData.shopId +
								'&status=1'
						})
					} else {
			...
			getshopdetail() {
				if (this.formData.shopId) {
					this.$request.get('/coc-active/api/v1/four_s/shopDetails/' + this.formData.shopId).then(res => {
						this.formData.brandId = res.data.brandId
			...
			onFoursShop() {
				uni.navigateTo({
					url: '/pages/upkeep/consult?isnew=1'
				})
			},
			OnFoursData(e) {
				const {
				...
				this.formData.shopName = storeAbbreviation
				this.meallist = []
				this.orderlist = []
			...
			OnpushMycar() {
				uni.navigateTo({
					url: '/pages/my/mycar/index?isupkeep=1' + "&brandId=" + this.formData.brandId
				})
			},
			getDetail() {
				if (this.formData.shopId) {
			...
			checkSale() {
				if (this.formData.shopId) {
					uni.navigateTo({
						url: "/pages/carShops/upkeep/checkSales?shopId=" + this.formData.shopId
					});
				} else {
					uni.$u.toast('请选择4S店')
				}
