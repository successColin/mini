<template>
  <view class="page">
    <view v-if="isExpert == 1 && token" class="fanyong" @click="Onpopposter">
      <view
        class="mainRed fwb tac"
        style="position: absolute; bottom: 0rpx; width: 100%"
      >
        得{{ commission }}元
      </view>
    </view>
    <view class="top"> </view>
    <view class="card">
      <view class="size32 fwb"> 救援项目说明 </view>
    ...
    <view class="bottom-btn" :style="{ bottom: tabbarHeight + 'px' }">
      ...
      <view
        class="txtLighGray size24 txtls relativtop6"
...
export default {
  ...
  methods: {
    getList() {
      this.$newrequest
        .post('/coc-active/api/v1/sky_you_product/details')
        .then((res) => {
    ...
    getcode() {
      ...
      this.$request
        .post('/coc-active/api/v1/invite/getMiniProgramQrCode', {
          ...
          scene: '1_' + uni.getStorageSync('userId') + '_enter=117',
        })
        .then((res) => {
