<template>
    ...
    <view class="page">
        <view v-if="isshowpage">
            ...
            <!-- 顶部广告栏 -->
            <carousel
                v-if="dataList.activity.adUrl || dataList.activity.adPushUrl"
                :list="getswiper()"
                height="200px"
                urlkey="url"
                dataType="1"
            ...
            <view
                ...
                v-if="dataList.activity.activityPaymentPreferenceId == 4"
            >
                <image
                    style="width: 650rpx; height: 58rpx"
            ...
            <view
                @click="Oncard(2)"
                style="text-align: center"
                v-if="dataList.activity.activityPaymentPreferenceId == 5"
            >
                <image
                    style="width: 650rpx; height: 58rpx"
            ...
            <view
                @click="Oncard(3)"
                style="text-align: center"
                v-if="dataList.activity.activityPaymentPreferenceId == 6"
            >
                <image
                    style="width: 650rpx; height: 58rpx"
            ...
            <view
                @click="Oncard(4)"
                style="text-align: center"
                v-if="dataList.activity.activityPaymentPreferenceId == 7"
            >
                <image
                    style="width: 650rpx; height: 58rpx"
            ...
            <view class="card" style="padding-top: 0rpx">
                <view class="card-item">
                    ...
                    <view class="activity-tags">
                        <view
                            v-for="(item, index) in dataList.activity.tagNamesList"
                            :key="index"
                    ...
                    <view class="u-flex mt20" style="align-items: flex-end">
                        <view class="price">
                            <text v-if="!dataList.activity.bankCardPrice">
                                <text class="size28 mainRed fwb">官方补贴价: ￥</text>
                                <text style="font-size: 40rpx">{{ dataList.activity.flashSalePrice }}</text>
                            </text>
                            <text v-else-if="!dataList.activity.flashSalePrice" style="font-size: 40rpx"> 免费 </text>
                            <text v-if="dataList.activity.bankCardPrice > 0">
                                <text
                                    v-if="dataList.activity.activityPaymentPreferenceId == 5"
                                    class="mainRed size24 ml20"
                                >
                                    农行信用卡专属价:￥
                                </text>
                                <text
                                ...
                                <text
                                    v-if="dataList.activity.activityPaymentPreferenceId == 8"
                                    class="mainRed size24 ml20"
                                >
                                    民生信用卡专属价:￥
                                </text>
                                <text
                                    v-if="dataList.activity.activityPaymentPreferenceId == 10"
                                    class="mainRed size24 ml20"
                                >
                                    官方补贴价:￥
                                </text>
                                <text class="mainRed fwb" style="font-size: 40rpx">
                                    {{ dataList.activity.bankCardPrice }}
                                </text>
                            </text>
                        </view>
                        <view style="font-size: 24rpx; color: #999999; margin-left: 20rpx">
                            <text style="text-decoration: line-through"
                                >市场价：¥{{
                                    dataList.activity.shopTypeId != 2 && dataList.activity.mgroupPrice != 0
                                        ? dataList.activity.mgroupPrice
                                        : dataList.activity.marketPrice
                                }}</text
                            >
                        </view>
                    </view>
                    <view class="jsb flex alc mt10">
                        <view class="deadline-text">报名截止时间：{{ dataList.activity.signUpEndTimeStr }}</view>
                        <view class="size24"> 已报名：{{ dataList.signUpCount }}/{{ dataList.allStock }} </view>
                    </view>
                ...
                <!-- 商家主页 -->
                <view class="card-item" @click="OnPushSHome">
                    <view class="flex jsb zhuye">
                        <view class="zhuyeL" style="width: 500rpx">
                            <view class="size28 fwb">{{ getshopName() }}</view>
                            <view class="mt20 size24 txtDarkGray">营业时间：{{ shopInfo.openingHours }} </view>
                        </view>
                        <view class="zhuyeR mainRed size24 tac">主页</view>
                    </view>
                    <view class="mt10 size24 txtDarkGray">{{ shopInfo.baseAddress }}</view>
                </view>
                <!-- 达人推荐 -->
                <view class="card-item" v-if="recommendLength">
                    <recommendX :fromType="2" :relatedType="1" :relatedId="id" :isTitle="2"></recommendX>
                </view>
                <floatVideo :relatedType="1" :relatedId="id"></floatVideo>
                <view v-if="dataList.activity.nonRefund == 1" class="mt20 mainRed"> 本套餐一经购买不支持退款 </view>
                <view class="card-item">
                    <!-- 活动发起人信息 -->
                    <!-- <view class="user-info"  @click="OnPushSHome">
						<view class="u-flex">
							...
							<view class="user-info-detail">
								<view class="name">{{ getshopName() }}</view>
							</view>
						</view>
					</view> -->
                    ...
                    <view class="content" v-else>
                        <view v-for="item in dataList.activity.details" :key="item.id">
                            ...
                            <view v-else-if="item.type == 2">
                                <image :src="item.associatedContent" mode="widthFix"></image>
                            </view>
                        </view>
                    </view>
                    <view v-if="imgs[0]" style="margin-top: 20rpx">
                        <view v-for="(item, index) in imgs" :key="index">
                            <image
                                style="width: 660rpx; margin-top: 10rpx; margin-left: 10rpx"
                                mode="widthFix"
                                :src="item"
                            >
                            </image>
                    ...
                    <view class="vote" v-if="dataList.activity.isInsertVote == 1">
                        ...
                        <view
                            v-for="(item, index) in dataList.activity.votesOptions"
                            :key="index"
                            class="option"
                            :class="choosevote === index ? 'choose' : undefined"
                            @click="OnChoosevote(item, index)"
                        >
                            <view>{{ item.optionContent }}</view>
                            <view
                                ><text v-if="item.percentage == null">0%</text><text v-else>{{ item.percentage }}</text>
                            </view>
                        </view>
                    </view>
                </view>
                ...
                <view class="card-item">
                    ...
                    <view class="mt20 size28">
                        <view class="txtLighGray">报名截止</view>
                        <view class="mt10">{{ dataList.activity.signUpEndTimeStr }}</view>
                    </view>
                    <view class="mt20 size28">
                        <view class="txtLighGray">活动场次</view>
                        <view
                            class="mt10"
                            v-for="(item, index) in dataList.activity.activityAttends"
                            :class="{ 'u-m-33': index != 0 }"
                            :key="index"
                            >{{ item.beginTime + " - " + item.endTime }}
                        </view>
                    </view>
                    <view @click="OnchooseLocation(2)" class="mt20 size28">
                        <view class="txtLighGray">活动地点</view>
                        <view class="mt10">
                            {{ dataList.activity.address }}
                        </view>
                    </view>
                    <view @click="Ontel()" class="mt20">
                        <view class="txtLighGray">联系方式</view>
                        <view>{{ dataList.activity.tel }}</view>
                    </view>
                </view>
                <view class="card-comments" style="margin-top: 20rpx" v-if="commentList.length">
                    <!-- 用户评价 -->
...
此说明仅当出现价格比较时有效。若商家单独对划线价格进行说明的，以商家的表述为准。
                        ...
                        <!-- <view class="li-item mt20">
                            商家详情页 (含主图)
                            以图片或文字形式标注的一口价、促销价、优惠价等价格可能是在使用优惠券、满减或特定优惠活动
                            和时段等情
                        </view>
                        <view class="li-item mt20">
                            此说明仅当出现价格比较时有效，若商家单独对划线价格进行说明的，以商家的表述为准。
                ...
                <!--  #ifdef MP-WEIXIN  -->
                <!-- 活动推荐 -->
                <view v-if="similarActivities.status" class="card-item">
                    <view class="title"> 活动推荐 </view>
                    <view
                        class="activity-item mt10"
                        v-for="(item, index) in similarActivities.list"
                        :key="index"
                    ...
                    >
                        <view class="u-flex" v-if="index < 4">
                            ...
                            <view
                                ...
                                "
                            >
                                <view class="activity-name moreLine"
            ...
            <view v-if="iscommission == 0" class="bottom-btn" style="border-top: 1rpx #e7e7e7 solid">
                <view class="view flex alc jsb" :style="{ paddingBottom: safeHeight > 0 ? 0 : '21rpx' }">
                    <view
                        class="u-flex"
                    ...
                    >
                        ...
                        >
                            ...
                            <view
                                v-if="dataList.activity.isCollection == 2 || dataList.activity.isCollection == null"
                    ...
                    <view
                        @click="Onremind"
                        v-else-if="dataList.activity.signUpStatus == 1"
                        class="signup1"
                    ...
                    <view
                        v-else-if="dataList.activity.activityEnd == 0"
                        class="signup"
                        :class="{ Asreminder: surplusStock == 0 }"
                        @click="Onsignup"
                    >
                        <text>立即报名</text>
                    </view>
            ...
            <image
                v-if="showtop"
                class="zhiding"
                src="https://oss.dcqcjlb.com/51che_java_dev/20240124/file_1706059356929.png"
                @click="ClickZhiDing"
            ></image>
                ...
                <view class="pop-singup">
                    <view class="flex" style="margin-bottom: -70rpx">
                        ...
                        >
                            <image
                                mode="aspectFill"
                                style="width: 184rpx; height: 184rpx; border-radius: 10rpx"
                                :src="dataList.activity.coverPicture"
                            ></image>
                        </view>
                        <!-- // 佣金 -->
                        <view style="margin-left: 20rpx">
                            <view>
                                <view class="flex alc">
                                    <view class="fwb size32 mainRed">
                                        ...
                                        ><text style="font-size: 40rpx">{{ flashSalePrice }}</text>
                                    </view>
                                    <view
                                        style="
                                            font-size: 20rpx;
                                        ...
                                        "
                                    >
                                        {{ mgroupPrice > 0 ? "市场价￥" + mgroupPrice : "￥" + marketPrice }}
                                    </view>
                                    <view
                                        class="u-flex ml20"
                                        v-if="
                            ...
                            <view
                                v-if="dataList.activity.bankCardPrice > 0"
                                class="size24 mt10"
                    ...
                    <view class="u-flex" style="flex-wrap: wrap; margin-top: 20rpx; margin-left: -20rpx">
                        <view
                            v-for="(item, index) in Meal.activitySetMeals"
                            :key="index"
                        ...
                        <view>
                            <scroll-view scroll-x="true" style="margin: 20rpx; font-size: 24rpx; width: 670rpx">
                                <view class="box-session">
                                    <view
                                        @click="OnSession(item, index)"
                                        v-for="(item, index) in Meal.activityAttends"
                                        :key="index"
                                        style="background: #f6f6f6; border-radius: 10rpx; margin-left: 30rpx"
                                    >
                                        <view
                                            style="padding: 20rpx; width: 140rpx"
                                            :class="{
                                                'disable-session': item.isCanSignUp == 0,
                                                'current-session': currentsession == index,
                                            }"
                                        >
                    ...
                    <view class="u-flex" style="justify-content: space-between">
                        <view style="font-size: 28rpx; margin-left: 20rpx"
                            >数量<text style="color: #d41b1b; font-size: 24rpx; margin-left: 10rpx" v-if="limitNum != 0"
                                >(限购{{ limitNum }}份)</text
                            ></view
                        >
                        <view class="u-flex">
            ...
            <view>
                <u-popup :show="appointVisble" @close="closeAppoint" mode="bottom">
                    <view class="appoint">
                        <text class="appoint-title" style="margin-bottom: 46rpx"
                            >客户参与活动时间意愿调查（可多选）</text
                        >
                        <view class="appoint-btn-view">
                            <view
                                v-for="(item, i) in appointList"
                                class="appoint-btn"
                                @click="clickAppoint(i)"
                                :key="i"
                                :style="{
            ...
            <u-popup
                ...
                :show="appointSuccessVisble"
                :closeOnClickOverlay="false"
                mode="center"
                customStyle="width:90%;borderRadius:20rpx"
            >
                <view class="appointsuccess">
                    <text class="appoint-title" style="margin-bottom: 48rpx; margin-top: 20rpx; :center "
                        >已收到您的需求，后续我们会考虑再次举办该活动，您也可以关注下我们其他活动。</text
                    >
            ...
            <u-popup round="10" :safeAreaInsetBottom="false" :show="openpop" @close="openpop = false" mode="center">
                <view style="text-align: center; padding: 40rpx 50rpx; width: 550rpx">
                    <view v-if="dataList.activity.activityPaymentPreferenceId == 4">
                        <view class="bank-title">投人保车险，享更多专属活动优惠 </view>
                        <view class="bank-btn">
                            <view @click="openpop = false" class="bankbtn-left">我已投保 </view>
                            <view @click="Oncard(1)" class="bankbtn-right">我要投保 </view>
                        </view>
                    </view>
                    <view v-else-if="dataList.activity.activityPaymentPreferenceId == 5">
                        <view class="bank-title">本活动报名支付时享农行信用卡专属满减优惠 </view>
                        <view class="bank-btn">
                            <view @click="creditcard(1)" class="bankbtn-left"> 我已有卡 </view>
                            <view @click="Oncard(2)" class="bankbtn-right"> 我要办卡 </view>
                        </view>
                    </view>
                    <view v-else-if="dataList.activity.activityPaymentPreferenceId == 6">
                        <view class="bank-title">本活动报名支付时享工行信用卡专属满减优惠 </view>
                        <view class="bank-btn">
                            <view @click="creditcard(2)" class="bankbtn-left"> 我已有卡 </view>
                            <view @click="Oncard(3)" class="bankbtn-right"> 我要办卡 </view>
                        </view>
                    </view>
                    <view v-else-if="dataList.activity.activityPaymentPreferenceId == 7">
                        <view class="bank-title">本活动报名支付时享邮储信用卡专属满减优惠 </view>
                        <view class="bank-btn">
                            <view @click="creditcard(2)" class="bankbtn-left"> 我已有卡 </view>
                            <view @click="Oncard(4)" class="bankbtn-right"> 我要办卡 </view>
                        </view>
                    </view>
                </view>
            </u-popup>
            <u-popup :closeable="true" round="8" :show="showsignup" mode="bottom" @close="closesignup">
                ...
                <view class="u-flex" style="margin-top: 20rpx; justify-content: center">
                    <view>
                        <u-radio-group v-model="radiovalue1" placement="column">
                            <u-radio activeColor="#D91B1B" :customStyle="{ marginBottom: '8px' }" :name="1"> </u-radio>
                        </u-radio-group>
                    </view>
                ...
                >
                    <view @click="addClue" class="addBtn tac size32 colorff bgRed"> 提交 </view>
                </view>
            </u-popup>
        </view>
    </view>
...
export default {
    ...
    onShareAppMessage(res) {
        tologin();
        this.$request
            .post("/coc-active/api/v1/marvellous_activity/share_v2", {
        ...
        let imageUrl = this.dataList.activity.appletsSharePic
            ? this.dataList.activity.appletsSharePic
            : this.dataList.activity.coverPicture;
        return {
            title,
            path:
                `/pages/home/activityDetail/index?forwardUserId=` +
                this.forwardUserId +
                "&id=" +
                this.id +
                "&enter=117",
    ...
    methods: {
        ...
        addClue() {
            ...
            this.$refs.uForm.validate().then((res) => {
                this.$newrequest
                    .post("/coc-active/api/v1/sign_up/not/designated/user/addClue", {
        ...
        OnPushSHome() {
            if (this.dataList.activity.shopTypeId == 2) {
                ...
                uni.navigateTo({
                    url: "/pages/activityMall/business/index?id=" + this.dataList.shopVO.shopId + "&relatedType=1",
                });
            }
        },
        Onremind() {
            ...
            uni.requestSubscribeMessage({
                tmplIds: ["DS1aWHTQ1gSPOeuh2-eng-mUZMX0xc_OMhFeJMESElY"],
                success(res) {
                    if (res["DS1aWHTQ1gSPOeuh2-eng-mUZMX0xc_OMhFeJMESElY"] == "accept") {
                        setstorage("setRegistrationreminder", "1");
                        _this.$request
                            .get("/coc-active/api/v1/activity/signUpStartRemind/" + _this.dataList.activity.id)
                            .then((res) => {
                                if (res.code == 200) {
                                    uni.showToast({
                                        icon: "none",
        ...
        popposter() {
            ...
            uni.requestSubscribeMessage({
                tmplIds: ["b4r77TQXXqIxfxxMx81L7Lp3pOhPKlAb7eVpp6mi7bY"],
                success(res) {},
            });
            this.$request
                .post("/coc-active/api/v1/invite/getMiniProgramQrCode", {
                    page: "pages/home/activityDetail/index",
        ...
        OnChooseMeal(item, index) {
            ...
            this.mgroupPrice = item.mgroupPrice;
        ...
        OnChoosevote(item, index) {
            this.$request
                .post("/coc-social/api/v1/article/vote", {
        ...
        Onsingup() {
            ...
            if (this.currentmealitem.newUserLimit == 1) {
                this.$request
                    .post("/coc-active/api/v1/group/isNewPerson", {
                        type: 1,
                    })
                    .then((res) => {
                        if (res.data.type == 1) {
        ...
        topay() {
            this.$request.get("/coc-active/api/v1/activity/getActivitySetMeal/" + this.id).then((res) => {
                if (res.data.activitySetMeals[this.MealIndex].surplusStock < this.activitySetMealNum) {
                    ...
                    uni.navigateTo({
                        url:
                            "/pages/service/payMent/detail?activityId=" +
                            this.id +
                            "&setMealId=" +
                            this.setMealId +
                            "&MealName=" +
                            ...
                            childnum +
                            "&flashSalePrice=" +
                            this.currentMeal.flashSalePrice +
                            "&title=" +
                            this.Meal.title +
                            "&day=" +
                            this.day +
                            ...
                            this.dataList.activity.activityPaymentPreferenceId +
                            "&forwardUserId=" +
                            this.forwardUserId,
                    });
                }
        ...
        next() {
            ...
            if (getstorage("token")) {
                ...
                _this.$request.get("/coc-active/api/v1/activity/getActivitySetMeal/" + _this.id).then((res) => {
                    _this.stock = res.data.stock;
                    _this.limitNum = res.data.limitNum;
                    _this.Meal = res.data;
                    _this.marketPrice = res.data.marketPrice;
                    _this.mgroupPrice = res.data.mgroupPrice;
        ...
        Onsignup() {
            ...
            if (this.dataList.activity.isDesignated == 1) {
                this.$newrequest
                    .post("/coc-active/api/v1/sign_up/designated/user/canBuy", {
                        activityId: this.id,
                        type: "1",
                    })
                    .then((res) => {
        ...
        getList() {
            ...
            let _this = this;
            this.$newrequest
                .post("/coc-active/api/v1/activity/detail", {
                    activityId: this.id,
                ...
                .then((res) => {
                    ...
                    if (res.data.activity.adPushUrl) {
                        swiperList.unshift({
                            ...
                            type: "video",
                            poster: res.data.activity.adCoverImg
                                ? res.data.activity.adCoverImg
                                : res.data.activity.adPushUrl + "?x-oss-process=video/snapshot,t_1000,m_fast",
                        });
                        this.autoPlay = false;
                    } else {
                        this.autoPlay = true;
                    ...
                    // 获取商家信息
                    this.getShopInfo(res.data.shopVO.shopId);
                    ...
                    if (res.data.activity.activityPaymentPreferenceId == 4) {
                        if (
                            !getstorage("poptype4") ||
                            new Date().getTime() - getstorage("poptype4") > 1000 * 60 * 60 * 24 * 30
                        ) {
                            this.openpop = true;
                    ...
                    // 农行弹框
                        ...
                        if (
                            !getstorage("poptype5") ||
                            new Date().getTime() - getstorage("poptype5") > 1000 * 60 * 60 * 24 * 30
                        ) {
                            this.openpop = true;
                    ...
                    // 工行弹框
                        ...
                        if (
                            !getstorage("poptype6") ||
                            new Date().getTime() - getstorage("poptype6") > 1000 * 60 * 60 * 24 * 30
                        ) {
                            this.openpop = true;
                    ...
                    // 邮储弹框
                        ...
                        if (
                            !getstorage("poptype7") ||
                            new Date().getTime() - getstorage("poptype7") > 1000 * 60 * 60 * 24 * 30
                        ) {
                            this.openpop = true;
        ...
        async getSimilarActivities() {
            if (this.dataList.activity.propert) {
                const { code, data, message } = await this.$request.post("/coc-active/api/v2/homePage/randomActivity", {
                    num: 1,
                    property: this.dataList.activity.property,
                });
                if (code === 200) {
                    this.similarActivities.status = true;
                    this.similarActivities.list = data;
                } else {
                    this.similarActivities.status = false;
                    this.similarActivities.list = [];
                    uni.$u.toast(message);
                }
            } else {
                this.similarActivities.status = false;
