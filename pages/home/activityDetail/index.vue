<template>
	<view class="page">
		<view v-if="isshowpage">
			<view v-if="isapp==1">
				<u-navbar :autoBack='false' bgColor='' leftText="" title=" " :safeAreaInsetTop="true"
					@leftClick='onreturn'>
			...
			<!--  #ifdef MP-WEIXIN  -->
			<!-- 人保横幅 -->
			<view @click="Oncard(1)" style="text-align: center;"
			...
			<!-- 农行横幅 -->
			<view @click="Oncard(2)" style="text-align: center;"
			...
			<!-- 工行横幅 -->
			<view @click="Oncard(3)" style="text-align: center;"
			...
			<!-- 邮储横幅 -->
			<view  @click="Oncard(4)" style="text-align: center;"
			...
			<view class="card" style="padding-top: 0rpx;">
				...
				<view class="activity-tags">
					<view v-for="(item, index) in dataList.activity.tagNamesList" :key="index"
					...
					<view class="price">
						<text v-if="!dataList.activity.bankCardPrice">
							<text style="font-size: 20rpx;">￥</text><text
								style="font-size: 40rpx;">{{ dataList.activity.flashSalePrice }}</text>
						</text>
						...
						<text v-if="dataList.activity.bankCardPrice>0">
							...
							<text v-if="dataList.activity.activityPaymentPreferenceId==6"
				...
				<view v-if="iscommission==1">
					<commission :price='dataList.activity.commissionPrice' :proportion='dataList.activity.rate'></commission>
				</view>
				...
				<view class="content" v-else="dataList.activity.details">
					<view v-for="(item, index) in dataList.activity.details" :key="item.id">
			...
			<view class="activity-item" v-for="(item, index) in dataList.similarActivities" :key="index"
				@click="OnPushDetail(item)" v-if="index < 4">
				<view class="u-flex">
					<u--image radius='12rpx' :showLoading="true" :src="item.coverPicture" width="159rpx"
						height="117rpx">
					</u--image>
					<view
			...
			<view v-else-if="iscommission==1" class="bottom-btn" style="border-top: 1rpx #e7e7e7 solid;">
				<view class="view " :style="{paddingBottom:safeHeight>0?0:'21rpx'}">
					<joinshopbtn :ids="dataList.activity.id" :type='1' :isSelection='dataList.activity.isSelection'></joinshopbtn>
				</view>
			</view>
			<!--  #endif -->
			...
			<u-popup :safeAreaInsetBottom='false' :show="showsingnup" @close="OnClosesingnup" mode="bottom">
				<view class="pop-singup">
					<view class="u-flex" style="margin-bottom: -70rpx;">
						<view
							...
							<view>
								<view class="u-flex alc">
									<view style="color: #d91b1b;font-size: 32rpx;font-weight: bold;">
										<text style="font-size: 20rpx;">￥</text><text
											style="font-size: 40rpx;">{{ flashSalePrice }}</text>
									</view>
									<view
					...
					<view class="u-flex" style="justify-content: space-between;">
						<view style="font-size: 28rpx;margin-left: 20rpx;">数量<text
								style="color: #D41B1B;font-size: 24rpx;margin-left: 10rpx;"
								v-if="limitNum != 0">(限购{{ limitNum }}份)</text></view>
						<view class="u-flex">
							<view @click="OnDelete"
			...
			<u-popup :safeAreaInsetBottom='false' :show="appointSuccessVisble" :closeOnClickOverlay="false"
				mode="center" customStyle="width:90%;borderRadius:20rpx">
