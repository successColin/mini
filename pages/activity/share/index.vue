<template>
	<view>
		<view class="register">
			...
			<view class="realPhone" v-show="!photoShow">
				<view class="phoneLook">
					<view class="phoneLookL">
						...
						<view class="phoneLookTxt">
							手机实名认证
						</view>
					</view>
					<button class="phoneBtn" openType="getPhoneNumber" @getphonenumber='getPhoneNumber'>一键认证</button>
				</view>
			...
			<view class="headItem" v-show="photoShow">
				<view class="headCustom">
					...
					<button class="checkPhoto" openType="chooseAvatar" @chooseavatar='OngetUserPhoto'>
						<image  src="@/pages/activity/static/image/upPhoto.png" mode="widthFix"
				...
				<view class="uname">
					<input class="nameInput" type='text' placeholder="请设置昵称" v-model='dataList.name' />
				</view>
			</view>
		</view>
		<view class="footer" v-show="photoShow">
			...
			<button class="nowBtn" @click="upInfo">
				立即登录
			</button>
		</view>
	...
	export default {
		...
		methods: {
			...
			async OngetUserPhoto(val) {
				...
				this.dataList.userPhoto = await this.uploadFilePromise(val.detail.avatarUrl)
			},
			OnPushPrivacyPolicy() {
				uni.navigateTo({
			...
			loginNow(phoneCode, openCode, photo, nick, register) {
				let that = this;
				uni.request({
						...
						if (res.data.code === 102) {
							...
							this.$refs.uToast.show({
								...
								complete() {
									...
									that.$xjlrequest.post('/user/login/get_center_tmp_token', {
										mobile: res.data.data.username
									}).then(s => {
										that.$xjlrequest.post(
											'/user/login/getXjlTokenByTmpToken', {
												tmp_token: s.data.access_token
											}).then(i => {
											uni.setStorageSync('xjltoken', i.data
												.token)
									...
									uni.setStorage({
										...
										success: function() {
											that.getinfo()
											if (getCurrentPages()[0].is ==
												"pages/activity/share/index") {
												uni.switchTab({
			...
			upInfo() {
				if (this.dataList.userPhoto && this.dataList.name && this.isAgree) {
					this.loginNow(this.phoneCode, this.openCode, this.dataList.userPhoto, this.dataList.name, 1)
