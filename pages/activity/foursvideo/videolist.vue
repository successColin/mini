	...
	<view class="page">
		<view style="position: fixed;top: 0rpx;">
			<video @click.stop="OnClickVideo" id="videoid" :autoplay='true'  :loop='true' :controls='false'
				style="width: 750rpx;height: 100vh;" :src="dataList[swiperindex].videoUrl" object-fit='contain' >
				</video>
		</view>
		<swiper @animationfinish='animationfinish' class="swiper" :vertical='true' style="height: 100vh;" @change='change' duration='300'>
			...
			<!-- 	<view style="height:100vh;z-index: 1;">
				...
				<view class="from-name">
					<view @click="Onpush4sdetail(item)">@{{item.type==4?item.shopName:'51车俱乐部'}}</view>
				</view>
				...
				<!-- 	<view @click="OnlikeBase(index)">
					...
					<!-- <view class="mt15" style="margin-bottom: 15rpx;">{{item.shareBaseNum}}</view>		
						<view  v-if="userId==item.userId"  @click="Onshowsheet" style="margin-left: 5rpx;">
				...
				<u-popup :safeAreaInsetBottom='false' :show="showreply" @close="showreply=false" mode="bottom">
					<view class="tac" style="height: 600rpx;width: 750rpx;padding: 40rpx 0rpx;">
						...
						<view class="tac return-btn" style="display: inline-block;" @click="Onreplypop(item)">提交评论</view>
					</view>
				</u-popup>
	...
	export default {
		...
		methods: {
			...
			change(e) {
				...
				this.changid = this.dataList[e.detail.current].id
				uni.$emit('changid',this.dataList[e.detail.current].id)
