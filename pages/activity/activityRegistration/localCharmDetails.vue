<template>
    <view class="page">
        <swiper class="swiper" :indicator-dots="true" :autoplay="autoplay" :circular="true" @change="change">
            <swiper-item v-for="(v, i) in details.imageList" :key="i">
                <image v-if="v.type === 'image'" :src="v.url" class="swiper" mode="aspectFit" />
                <video
        ...
        <view v-if="tabList.length" class="tabs">
            <view class="tabs-flex">
                <view v-for="(v, i) in tabList" :key="i" class="tabs-item" @click="setTabs(v)">
                    <view :class="v.index === tabIndex ? 'tabs-item-title' : 'tabs-item-untitle'">{{ v.name }}</view>
                    <view :class="v.index === tabIndex ? 'tabs-item-line' : 'tabs-item-unline'" />
                </view>
            </view>
        </view>
        <view v-if="list.length" class="shop">
            <view v-for="(v, i) in list[tabIndex]" :key="i" class="shop-item" @click="toShop(v)">
                ...
                <scroll-view class="shop-flex" scroll-x>
                    <view v-for="(value, index) in v.imageList" :key="index" class="shop-item-dib">
                        <image v-if="value.shopCarouselType === 1" :src="value.carouselUrl" class="shop-item-image" />
                        <video
        ...
        <view class="recommend">
            ...
            <view class="recommend-list">
                <view v-for="(v, i) in details.list" :key="i" class="recommend-item" @click="toDetails(v)">
...
export default {
    ...
    onShareAppMessage() {
        const { id } = this;
        let path = "/pages/activity/activityRegistration/localCharmDetails?id=" + id;
        this.setShare(3);
        return {
            path: path,
            success() {
    ...
    methods: {
        ...
        async getLocalCharmDetail(localCharmId) {
            const {
                data: { title, address, imgs, video, videoImg, detail, localCharmList, likeType },
        ...
        async getShopList(localCharmId) {
            const { data } = await this.$request.post("/coc-active/api/v1/localCharm/getLocalCharmDetailShopList", {
                localCharmId,
            });
            const list = [[], [], []];
            data.forEach((v) => {
                ...
                if (v.categoryParentId === 19) {
                    list[0].push(v);
                } else if (v.categoryParentId === 15 || v.categoryParentId === 17 || v.categoryParentId === 18) {
                    list[1].push(v);
        ...
        async getCommentList(current) {
            const {
                data: { records, total },
            } = await this.$request.post("/coc-system/api/v1/user_interaction/getComment", {
                id: this.id,
                commentType: 8,
                current,
                size: 10,
        ...
        async setLike(isLike) {
            if (!this.statusLike) {
                ...
                const { code } = await this.$request.post("/coc-system/api/v1/user_interaction/like", {
                    id: this.id,
                    isLike,
                    likeType: 5,
                });
        ...
        async setShare(shareType) {
            const { code, message } = await this.$request.post("/coc-system/api/v1/user_interaction/share", {
                id: this.id,
                shareType,
            });
            if (code !== 200) {
