				...
				<view class="tab_inner">
					<view class="tab_div flex jsb">
						...
						<view class="tab_item" :class="{'tab_item_sel':type===1,'tab_item_nosel':type!==1}"
							@click="OnClickStatus(1)">更多玩乐</view>
		...
		methods: {
			...
			toList(value) {
				...
				if (value.video) {
					type = 1
					uni.navigateTo({
						url: '/pages/activity/waterfull/videolist?id=' + value.articleId+'&selectionType=2',
					})
				} else {
					uni.navigateTo({
						url: '/pages/activity/waterfull/imglist?id=' + value.articleId+'&selectionType=2',
					})
				}
			}
		}
