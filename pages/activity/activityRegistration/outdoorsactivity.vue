<template>
    <view class="page">
        <view class="bgfff page_top">
            <search :fromType="'户外'" :isIcon="1" @getValue="getValue" :isBgf="true"></search>
            <view class="tabs">
                    ...
                    <view class="tabs-item">
                        <view class="tabs-item-right">
                            <view class="tabs-item-view" @click="getStatus(false)">
                                <view :class="!status ? 'tabs-item-title' : 'tabs-item-untitle'">周边度假</view>
                                <image
                                    v-show="!status"
                                    src="https://oss.dcqcjlb.com/51che_java_dev/20231027/file_1698392764793.png"
                                    class="tabs-item-image"
        ...
        <view>
            ...
            <template v-if="!status">
                <view v-if="dataList.length">
                    <view v-for="(item, index) in dataList" :key="item.shopId" class="card-list" @click="OnPush(item)">
...
export default {
    ...
    props: {
        fromType: Number, // 17 户内  18 户外  19 团购
    },
    data() {
        return {
            showTabs: false,
    ...
    methods: {
        ...
        getTabList() {
            this.$request
                .post("/coc-active/api/v2/homePage/shopCategoryList", { type: 2, parentId: 18 })
                .then((res) => {
        ...
        //周边度假分类
        getMoreTabList() {
            this.$request.post("/coc-active/api/v2/homePage/shopCategoryList", { parentId: 18 }).then((res) => {
                res.data = res.data.map((m) => {
                    return {
                        name: m.shopCategoryName,
                        id: m.shopCategoryId,
        ...
        getactivity() {
            this.$newrequest
                ...
                .then((res) => {
                    if (res.code == 200) {
                        if (res.data.homePageActivitysVOIPage.records.length < 10) {
...
.whiteBg {
    padding: 25rpx 25rpx 0 !important;
}
