				...
				<scroll-view scroll-y="true" @scrolltolower="scrollBottom" :style="{ maxHeight: height }">
					...
					<view class="select">
						...
						<view class="select-content">
									...
									<view class="select-activity-item" v-for="(item, index) in dataList"
										...
										<view class="u-flex-al jsb bgf8"
											...
											<view v-else-if="selectIndex == 2&&iscommission==1"
												...
												class="select-activity-item-btn u-flex" style="background: #d91b1b"
												:class="{'bg999':item.isSelection==1}">加小店
											</view>
											<view v-if="selectIndex == 3 && item.isLeaveMessage == 1"
												@click.stop="openAppoint(item.id)"
												class="select-activity-item-btn u-flex" style="background: #d91b1b">我想参加
											</view>
											<view v-if="selectIndex == 3 && item.isLeaveMessage == 0"
												class="select-activity-item-btn u-flex" style="background: grey"
												@click.stop>
												我想参加</view>
										</view>
									</view>
							...
							<view v-if="selectIndex==6">
								...
								<view v-for="(item, index) in dataList" :key="item.productId" @click="Onpushdanpin(item)">
										<view class="card" style="display: flex;">
											<view>
												<image style="width: 107rpx;height: 107rpx;border-radius: 8rpx;" :src="item.coverPicture"></image>
											</view>
											<view class="ml10">
												...
												<view style="margin-left: 25rpx;">
													...
													<view class="flex alc jsb" style="margin-top: 30rpx;">
														...
														<view class="tac">
															...
															<view v-else-if="item.isEnd==1&&iscommission==0"
																class="buy-btn" :class="{ graybtn: true }">
																已结束
															</view>
							...
							<view
								v-if="selectIndex == 4 && (detail.shopTypeDetails == 1 || detail.shopTypeDetails == 3)"
								class="select-wonderfulreview">
								<!--  #ifdef MP-WEIXIN  -->
								<view v-if="dataList.length">
									<custom-waterfalls-flow imageKey="coverImg" ref="waterfallsFlowRef"
										:value="dataList" @imageClick="jumpWonderfulReviewDetail">
										<view class="select-wonderfulreview-item" v-for="(item, index) in dataList"
											:key="index" slot="slot{{index}}" @click="jumpWonderfulReviewDetail(item)">
