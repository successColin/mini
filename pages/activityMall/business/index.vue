...
<view @click="previewImg" class="tac"> 
									<image style="width: 46rpx;height: 46rpx;" src="https://oss.dcqcjlb.com/51che_java_dev/20231009/file_1696839871028.png"></image>
            ...
            ">
						<view class="tab_item" style="line-height: 100rpx;margin-right: 60rpx;" v-for="(item, index) in newtags" :key="index"
                        ...
                        " :src="item.coverPicture">
										...
										<view style="margin-left: 15rpx">
											...
											<view class="u-flex alc mt10">
												...
												<view class="size24 txtLighGray ml10 txtls" >¥{{ item.mgroupPrice?item.mgroupPrice:item.marketPrice}}
												</view>
											</view>
										</view>
									</view>
							...
							<view class="main1">
								...
								<view v-if="dataList.groupDisplayVOS.length > 0" class="mt10">
									<view v-for="item in dataList.groupDisplayVOS" :key="item.id"
							...
							<view class="main2">
								...
								<view>
									<view v-if="dataList.activityDisplayVOS.length > 0" class="mt10">
										<view v-for="item in dataList.activityDisplayVOS" :key="item.id"
											...
											<scroll-view scroll-x="true" scroll-left="0">
												<view class="u-flex">
													<view v-for="item in dataList.marvellousVOS"
														:key="item.id" style="margin-right: 23rpx"
	...
	export default {
		...
		methods: {
			...
			Onpushchant(item) {
				...
				console.log(item)
				uni.navigateTo({
					url: "/pages/grouppurchase/packagedetail?id=" + item.id,
				});
			},
			getheight() {
				...
				this.$nextTick(() => {
					...
					arr.forEach((item, index) => {
						...
						query.select('.main' + index).boundingClientRect(v => {
							if (item.index == 1) {
								if (v.height != 0) {
									this.newtags.push({
										name: '精选套餐',
			...
			// 跳转锚点
			goto(name) {
				uni.createSelectorQuery().select(name).boundingClientRect(data => { //目标位置的节点：类class或者id
					uni.createSelectorQuery().select(".u_contetns").boundingClientRect(res => { //最外层盒子的节点：类class或者id
						uni.pageScrollTo({
							duration: 100, //过渡时间
