	...
	<view class="page">
		<view v-if="isshowpage">
			<view style="text-align: center;margin-top: 15rpx;">
				<image @click="watchimg(imglist[index])" style="width: 473rpx;height: 726rpx;border-radius: 15rpx;"
		...
		<view class="bottom-card">
			<view>
				<scroll-view class="scroll-view_H" scroll-x="true">
					<view class="u-flex">
						<view v-for="(item,index) in imglist" :key="index" style="margin-right: 23rpx	;">
							<image @click="OnChooseImg(index)" style="width: 123rpx;height: 184rpx;border-radius: 8rpx;"
								:src="item"></image>
						</view>
					</view>
				</scroll-view>
...
<script>
	export default {
		...
		onShareAppMessage(res) {
			let title = uni.getStorageSync('nickname')+'邀请您升级51车俱乐部达人'
			return {
				title,
				imageUrl:'https://oss.dcqcjlb.com/51che_java_dev/20230526/file_1685064278696.jpg',
				path: `/pages/wiseman/welcome?expertParentId=` + uni.getStorageSync('userId'),
				success(res) {
		...
		onShareTimeline(res) { //分享到朋友圈
			return {
				title: uni.getStorageSync('nickname')+'邀请您升级51车俱乐部达人',
				imageUrl:'https://oss.dcqcjlb.com/51che_java_dev/20230526/file_1685064278696.jpg',
				path: `/pages/wiseman/welcome?expertParentId=` + uni.getStorageSync('userId'),
				success(res) {
					//console.log('success(res)==', res);
		...
		methods: {
			...
			getList() {
				...
				this.$request.post("/coc-active/api/v1/newExpert/invite/list").then(res => {
