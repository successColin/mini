    ...
    <view v-if="isshowpage">
      <view style="position: fixed; top: 0rpx">
        <video
          ...
          :src="dataList[swiperindex].article.video"
          object-fit="contain"
        ></video>
      </view>
      <swiper
          ...
          <swiper-item v-for="(item, index) in dataList" :key="index">
            ...
            <view class="shop-style">
              ...
              >
                <view class="u-flex alc">
                  ...
                  <image
                    v-if="item.expertShopSelection.type == 5"
                    style="width: 32rpx; height: 32rpx"
                    src="https://oss.dcqcjlb.com/51che_java_dev/20230603/file_1685779116967.png"
                  >
                  </image>
              ...
              <view
                v-else-if="item.expertSelectionLibrary"
                class="u-flex background-touming"
                @click="Onpushdetail(item)"
              >
                <view class="u-flex alc">
                  ...
                  <image
                    v-if="item.expertSelectionLibrary.type == 6"
                    style="width: 32rpx; height: 32rpx"
                    src="https://oss.dcqcjlb.com/51che_java_dev/20230816/file_1692147614258.png"
                  >
                  </image>
                  <image
                    v-if="item.expertSelectionLibrary.type == 2"
                    style="width: 32rpx; height: 32rpx"
                    src="https://oss.dcqcjlb.com/51che_java_dev/20230816/file_1692147614258.png"
                  >
                  </image>
                  <image
                    v-if="item.expertSelectionLibrary.type == 3"
                    style="width: 32rpx; height: 32rpx"
                    src="https://oss.dcqcjlb.com/51che_java_dev/20230823/file_1692770227497.png"
                  >
                  </image>
                  <image
                    v-if="item.expertSelectionLibrary.type == 1"
                    style="width: 32rpx; height: 32rpx"
                    src="https://oss.dcqcjlb.com/51che_java_dev/20230603/file_1685779655989.png"
                  >
                  </image>
                  <image
                    v-if="item.expertSelectionLibrary.type == 4"
                    style="width: 32rpx; height: 32rpx"
                    src="https://oss.dcqcjlb.com/51che_java_dev/20230603/file_1685779116967.png"
                  >
                  </image>
                  ...
                  <image
                    v-if="item.expertSelectionLibrary.type == 5"
                    style="width: 32rpx; height: 32rpx"
                    src="https://oss.dcqcjlb.com/51che_java_dev/20230603/file_1685779116967.png"
                  >
                  </image>
              ...
              <view
                v-if="item.article.address"
                class="ml10"
                @click="Onpushmap(item)"
              >
            ...
            <view class="area">
              <view @click="OnlikeBase(index)">
                <image
                  v-if="item.article.likeType == 2"
                  style="width: 61rpx; height: 53rpx"
              ...
              <view class="mt40">
                ...
                >
                  <image
                    style="width: 57rpx; height: 46rpx"
                    src="https://oss.dcqcjlb.com/51che_java_dev/20230817/file_1692237952655.png"
                  >
                  </image>
                </button>
              </view>
              <view class="mt15" style="margin-bottom: 15rpx">{{
                item.article.shareBaseNum
              }}</view>
              <view
                @click="Onshowsheet"
                v-if="item.article.userId == userId"
            ...
            >
              <view style="height: 600rpx; padding: 40rpx 55rpx 20rpx 25rpx">
                <view class="tac size24 fwb mb20"
                  >{{ item.article.commentNum }}条评论</view
                >
                <scroll-view
                  scroll-y="true"
                  v-if="item.comments"
                  style="min-height: 0rpx; max-height: 400rpx"
                >
                  <view class="comment-list">
                    ...
                    >
                      <view class="comment">
                        ...
                        >
                          ...
                          <view
                            ...
                            @click.stop="delComments(item1.commentId)"
                            style="
                              color: #d91b1b;
                              margin-top: 16rpx;
                              font-size: 28rpx;
                      ...
                      <view
                        v-for="(item2, index2) in item1.replyVOS"
                        :key="index2"
                ...
                <view
                  style="position: fixed; bottom: 50rpx; width: 700rpx"
                  @click="showcommentpop = true"
                >
                  <u--input
                    shape="circle"
                    :disabled="true"
            ...
            >
              <view
                class="tac"
                style="height: 600rpx; width: 750rpx; padding: 40rpx 0rpx"
              >
            ...
            >
              ...
              >
                ...
                <view
                  class="tac return-btn"
                  style="display: inline-block; margin-top: -10rpx"
                  @click="Onreplyfirst"
                  >提交评论</view
                >
              </view>
...
export default {
  ...
  onLoad(option) {
    ...
    let _this = this;
    uni.createVideoContext('play0').play();
  ...
  onShareAppMessage(res) {
    ...
    return {
      title: title,
      path:
        `/pages/wiseman/component/videoarticle?id=` +
        this.changid +
        '&enter=117',
      success(res) {
        uni.showToast({
  ...
  methods: {
    ...
    delComments(id) {
      let _this = this;
      this.$request
        .post('/coc-active/api/v1/comments/delComments', {
          id: id,
        })
        .then((res) => {
          if (res.code == 200) {
    ...
    Onreplyfirst() {
      ...
      this.$request
        .post('/coc-social/api/v2/article/comment', {
          commentType: 2,
        ...
        .then((res) => {
          if (res.code == 200) {
            if (this.dataList[index].article.likeType == 2) {
              this.dataList[index].article.likeType = 1;
              this.dataList[index].article.likeBaseNum =
                this.dataList[index].article.likeBaseNum + 1;
            } else {
              this.dataList[index].article.likeType = 2;
              this.dataList[index].article.likeBaseNum =
                this.dataList[index].article.likeBaseNum - 1;
            }
          } else {
    ...
    getList() {
      this.$request
        .post('/coc-social/api/v2/article/ids', {
          id: this.id,
        })
        .then((res) => {
    ...
    OnClickVideo(index) {
      if (!this.isplay) {
        this.isplay = true;
        uni.createVideoContext('play' + index).pause();
      } else {
        this.isplay = false;
        uni.createVideoContext('play' + index).play();
      }
    },
    Onpushdetail(item) {
      let id = item.expertShopSelection
        ? item.expertShopSelection.relatedId
        : item.expertSelectionLibrary?.relatedId;
      // 2保养套餐  3救援卡 1活动 4团购 5单品套餐6车款
      if (item.expertShopSelection) {
        if (
          item.expertShopSelection.type == 2 ||
          item.expertSelectionLibrary?.type == 2
        ) {
          uni.navigateTo({
            url:
          ...
          item.expertShopSelection.type == 3 ||
          item.expertSelectionLibrary?.type == 3
        ) {
          uni.navigateTo({
            url: '/pages/roadhelp/index?forwardUserId=' + item.article.userId,
          });
        } else if (
          item.expertShopSelection.type == 1 ||
          item.expertSelectionLibrary?.type == 1
        ) {
          uni.navigateTo({
            url:
          ...
          item.expertShopSelection.type == 4 ||
          item.expertSelectionLibrary?.type == 4
        ) {
          uni.navigateTo({
            url:
          ...
          item.expertShopSelection.type == 5 ||
          item.expertSelectionLibrary?.type == 5
        ) {
          uni.navigateTo({
            url:
              '/pages/activity/activityRegistration/packagedetail?id=' +
              id +
              '&forwardUserId=' +
              item.article.userId,
          });
        ...
        if (item.expertSelectionLibrary?.type == 2) {
          ...
          uni.navigateTo({
            url:
              ...
              item.expertSelectionLibrary?.relatedId +
              '&type=1&shopId=' +
              item.expertSelectionLibrary?.shopId,
          });
        }
      }
    },
    change(e) {
      ...
      this.commentList = this.isplay = false;
      uni.createVideoContext('play' + this.swiperindex).stop();
      this.swiperindex = e.detail.current;
      uni.createVideoContext('play' + this.swiperindex).play();
