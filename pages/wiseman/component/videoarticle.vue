<template>
	<view class="page">
		...
		<swiper @animationfinish='animationfinish' class="swiper" :vertical='true' style="height: 100vh;" @change='change' duration='300'>
		...
		<view v-else-if="item.expertSelectionLibrary" class="u-flex background-touming" @click="Onpushdetail(item)">
						<view class="u-flex alc">
							<!-- 2保养套餐  3救援卡 1活动 4团购 5单品套餐6车款-->
							<image v-if='item.expertSelectionLibrary.type==6' style="width: 32rpx;height: 32rpx;"
								src="https://oss.dcqcjlb.com/51che_java_dev/20230816/file_1692147614258.png">
							</image>
							<image v-if='item.expertSelectionLibrary.type==2' style="width: 32rpx;height: 32rpx;"
								src="https://oss.dcqcjlb.com/51che_java_dev/20230816/file_1692147614258.png">
							</image>
				...
				<view class="area">
					...
					<view   class="mt15" style="margin-bottom: 15rpx;">{{item.article.shareBaseNum}}</view>		
						<view  @click="Onshowsheet" v-if="item.article.userId == userId" style="margin-left: 5rpx;">
...
<script>
	...
	export default {
		...
		onShareAppMessage(res) {
			...
			return {
				title: title,
				path: `/pages/wiseman/component/videoarticle?id=` + this.changid,
				success(res) {
					uni.showToast({
						title: '分享成功'
					})
				...
				this.$request.post("/coc-social/api/v2/article/like", {
					...
					if (res.code == 200) {
						...
						if (this.dataList[index].article.likeType == 2) {
							...
							this.dataList[index].article.likeBaseNum = this.dataList[index].article.likeBaseNum + 1
						} else {
		...
		Onpushdetail(item) {
			...
			if(item.expertShopSelection){
				if (item.expertShopSelection.type == 2||item.expertSelectionLibrary.type == 2) {
					uni.navigateTo({
						url: '/pages/carShops/upkeep/detail?id=' +id +
							'&forwardUserId=' + item.article.userId
					})
				} else if (item.expertShopSelection.type == 3||item.expertSelectionLibrary.type == 3) {
					uni.navigateTo({
						url: '/pages/roadhelp/index?forwardUserId=' + item.article.userId
					})
				} else if (item.expertShopSelection.type == 1||item.expertSelectionLibrary.type == 1) {
					uni.navigateTo({
						url: '/pages/home/activityDetail/index?id=' +id +
							'&forwardUserId=' + item.article.userId
					})
				} else if (item.expertShopSelection.type == 4||item.expertSelectionLibrary.type == 4) {
					uni.navigateTo({
						url: '/pages/grouppurchase/detail?id=' + id +
							'&forwardUserId=' +
							item.article.userId
					})
				} else if (item.expertShopSelection.type == 5||item.expertSelectionLibrary.type == 5) {
					uni.navigateTo({
