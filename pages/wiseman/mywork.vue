	...
	<view class="page">
		<view class="top">
			<view style="width: 700rpx;">
				<u-search @custom="tosearch" @search='tosearch' placeholder="搜索作品标题" v-model="title"
				...
				<custom-waterfalls-flow imageKey='coverPicture' ref="waterfallsFlowRef" :value="dataList"
					...
					<view v-for="(item,index) in dataList" :key="item.articleId" slot="slot{{index}}"
						...
						<view v-if="item.status!=1" @click="Ondetail(item)">
							<image style="width: 46rpx;height: 48rpx;position: absolute;right: 0rpx;top: 0rpx;"
								src="https://oss.dcqcjlb.com/51che_java_dev/20230928/file_1695867520236.png"></image>
						</view>
						...
						<view class="waterfall-title" :class="{'mt-40':item.type||item.citeType}">
						...
						<view v-if="item.status==1" class="u-flex jsb ml10 mr10 size20">
							<view class="u-flex alc">
								<view class="u-flex alc">
									<image style="width: 25rpx;height: 19rpx;"
							...
							<view class="u-flex alc">
								<view class="u-flex alc">
									<image style="width: 20rpx;height: 22rpx;"
										src="https://oss.dcqcjlb.com/51che_java_dev/20230816/file_1692146821761.png">
									</image>
								</view>
								<view style="margin-left: 5rpx;">{{item.expertSaleCount}}</view>
							</view>
						</view>
						<view v-else class="size20 ml10">
							<view v-if="item.status==4">待审核</view>
							<view v-else-if="item.status==3">屏蔽原因：{{item.shieldedReason}}</view>
						</view>
					</view>
		...
		<u-modal :show="showmodal" @confirm='detailarcitle' :showCancelButton='true' content='确认删除贴文?'
