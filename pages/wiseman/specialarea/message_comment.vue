					...
					<view class="item_right flex jsb">
						<view class="item_info">
							...
							<view class="item_content">
								...
								<view v-if="item.commentImgsList.length>0" class="flex alc" style="margin-top: 15rpx;"
							...
							<view v-if="item.type==5" class="item_my_div">
								...
								<view v-if="item.myCommentImgsList.length>0" class="flex alc" style="margin-top: 15rpx;"
			...
			<view class="pop">
				<view class="card">
					<view style="margin-top: 20rpx;">
						<u--textarea maxlength='500' v-model="chosecomment.content" placeholder="请输入评论内容"></u--textarea>
					</view>
				</view>
				<view style="margin: 40rpx;">
					<u-upload :fileList="fileList" @delete="deletePic" @afterRead="afterRead" :maxCount="1">
					</u-upload>
				</view>
				<view class="Totrue">
		...
		methods: {
			...
			submitcomments() {
				if (this.isend) {
					...
					this.$request.post("/coc-social/api/v2/article/comment", Object.assign({
						imgs
					}, this.chosecomment)).then(res => {
						if (res.code == 200) {
			...
			jumpDetail(item) {
				...
				if (item.videoList && item.videoList.length > 0) {
					...
					url='/pages/activity/waterfull/videolist?id='+item.articleId
				} else {
