<template>
	<view class="page">
		<carousel :topStyle="{ marginTop: '23rpx' }" :radius="7" height="256rpx" urlkey="url" :dataType="2"
			systemCode="APPLETS_CREATIVE_CENTER_TOP" :indicatorType="1">
		</carousel>
		<view class="joingroup flex jsb alc">
			<view class="flex alc">
				<image
					src="https://oss.dcqcjlb.com/51che_java_dev/20230816/file_1692167968026.png"
					class="join_icon"></image>
		...
		<view class="inspiration" v-if="topicList.length>0">
			...
			<scroll-view class="tabs_div" :scroll-x="true">
				<view class="tabs flex alc">
					<view v-for="item in tagList" class="tab_item flex alc"
						:class="{'item_sel':curIndex===item.id,'item_nosel':curIndex!==item.id}"
			...
			<view class="inspiration_list">
				<template v-for="item in topicList">
					<view v-if="item.baseDataDictionaryIdContextType===curIndex" :key="item.id"
						class="inspiration_item flex jsb alc">
						<view class="flex alc">
							<image class="item_img" :src="item.coverImage">
							</image>
						...
						<view style="width: 127rpx;" @click="jumpAddPost(item)">
							<u-button type="error" :plain="true" shape="circle" text="去发布"
		...
		<view class="card" v-for="(item,index) in dataList" :key="item.id">
			...
			<view class="actrice_list">
				<view v-for="(item1,index1) in item.article" :key="item1.id" class="actrice_item u-flex jsb"
					@click="()=>OnPushdetail(item1)">
					<view class="article_title moreLine">{{item1.title}}
					</view>
	...
	export default {
		...
		data() {
			return {
				...
				topicTitle: "",
				isshow: false,
				showimg: 'https://oss.dcqcjlb.com/51che_java_dev/20230826/file_1693017167078.jpg'
			}
		},
		onLoad() {
			this.queryTopicList()
		...
		methods: {
			...
			//查询话题列表
			queryTopicList() {
				this.$newrequest.post("/coc-active/api/v1/creative/center/topic/context_type/list").then(res => {
					if (res.code === 200) {
						res.data.forEach(m => {
			...
			//查询文章列表
			queryActriceList() {
				this.$newrequest.post("/coc-active/api/v1/creative/center/home", {
					"isAllType": 0
				}).then(res => {
					if (res.code === 200) {
