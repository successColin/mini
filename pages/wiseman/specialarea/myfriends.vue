	...
	<view class="page">
		<view class="firend_tab">
			<view class="tab_div flex jsb">
				<view class="tab_item" :class="{'tab_item_sel':currentIndex===1,'tab_item_nosel':currentIndex===2}"
					@click="OnClickType(1)">关注</view>
				<view class="tab_item" :class="{'tab_item_sel':currentIndex===2,'tab_item_nosel':currentIndex===1}"
					@click="OnClickType(2)">粉丝</view>
			</view>
			<view class="tab_line" :class="{'tab_tran1':currentIndex===1,'tab_tran2':currentIndex===2}"></view>
		</view>
		<view v-if="dataList.length>0" class="firend_list">
			<view v-for="item in dataList" :key="item.userId" class="list_item">
				<view class="flex alc jsb">
					...
					<view v-if="item.status==1" class="list_item_btn btn2" @click="follow(item)">
						<text>取消关注</text>
					</view>
					<view v-if="currentIndex===2&&item.status==0" class="list_item_btn btn1" @click="follow(item)">
						<text>回关</text>
					</view>
				</view>
				<view>
		...
		<view v-else class="tac mt50">
			...
			<view class="txtDarkGray">暂无{{currentIndex==1?'关注':'粉丝'}}</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			queryList() {
				this.dataList = []
				this.$request.post("/coc-social/api/v2/article/friendList", {
