    ...
    <view class="square" :class="mh100">
        <view v-if="!show" class="square-top" :class="home ? 'pt23' : ''">
            ...
            <view class="df aic" :class="home ? 'mr15' : ''" @click="toXPGC">
                ...
                <image src="https://oss.dcqcjlb.com/51che_java_dev/20230816/file_1692170166388.png" class="w46h46" />
            </view>
        </view>
        <view v-else>
            <view class="h23" />
        ...
        <view v-for="(v, i) in list" :key="i" class="square-item" @click="setList(v)">
            <image :src="v.coverPicture" class="square-item-image" />
            <view>
                <view class="square-item-name">{{ v.relatedName }}</view>
                <view class="df">
                    <view v-if="v.activityPaymentPreferenceId === 4" class="square-item-label mt8">
                        <view class="square-item-label-title">人保信用卡用户专享</view>
                    </view>
                    <view v-else-if="v.activityPaymentPreferenceId === 5" class="square-item-label mt8">
                        <view class="square-item-label-title">农行信用卡用户专享</view>
                    </view>
                    <view v-else-if="v.activityPaymentPreferenceId === 6" class="square-item-label mt8">
                        <view class="square-item-label-title">工行信用卡用户专享</view>
                    </view>
					<view v-else-if="v.activityPaymentPreferenceId === 7" class="square-item-label mt8">
					    <view class="square-item-label-title">邮储信用卡用户专享</view>
					</view>
					<view v-else-if="v.activityPaymentPreferenceId === 8" class="square-item-label mt8">
					    <view class="square-item-label-title">民生信用卡用户专享</view>
					</view>
					<view v-else-if="v.activityPaymentPreferenceId === 10" class="square-item-label mt8">
                ...
                <view class="df jcsb w504h156">
                    <view class="w419h156">
                        <view v-if="v.marketPriceStr" class="df mt8">
                            <view class="square-item-red">¥{{ v.bankCardPriceStr || v.flashSalePriceStr }}</view>
                            <view class="square-item-grey">¥{{ v.marketPriceStr }}</view>
                        </view>
                        <view v-else class="df mt8">
                            <view v-if="v.type_txt" class="square-item-treetop">{{ v.type_txt }}</view>
                            <view class="square-item-red">¥{{ v.bankCardPrice || v.flashSalePrice }}</view>
                            <view class="square-item-grey">¥{{ v.marketPrice }}</view>
                        </view>
                        <view v-if="v.shopName" class="square-item-business" @click.stop="toShop(v)">
                            <image :src="v.baseHead" class="w38h38" />
                            <view class="square-item-business-title">{{ v.shopName.length > 8 ? v.shopName.substring(0, 8)
                                + '...' : v.shopName }}</view>
                            <view v-if="v.shopId !== 1" class="square-item-business-black">进店</view>
                            <image v-if="v.shopId !== 1" src="@/static/image/vector.svg" class="w6h12" />
                        </view>
                        <view v-if="isshow" class="df aife">
                            <view class="df aife">
                                <view class="square-item-money">¥</view>
                                <view class="square-item-money2">{{ v.commissionPrice }}</view>
                                <view class="square-item-money">单笔赚</view>
                            </view>
                            <view class="square-item-commission">{{ v.rate }}%佣金率</view>
                        </view>
                    </view>
                    <view v-if="isshow">
                        <view v-if="v.isSelection" class="w77h81" @click.stop="setDelete(v)">
                            <image src="https://oss.dcqcjlb.com/51che_java_dev/20230819/file_1692437414837.png"
                                class="w48h48" />
                            <view class="square-item-delete">移出小店</view>
                        </view>
                        <view v-else class="w77h81" @click.stop="setAdd(v)">
                            <image src="https://oss.dcqcjlb.com/51che_java_dev/20230816/file_1692170202051.png"
                                class="w48h48" />
        ...
        <view v-if="list.length === 0" class="square-empty">
            <image src="https://oss.dcqcjlb.com/51che_java_dev/20230822/file_1692689603184.png"
...
export default {
    ...
    created() {
        if (!this.isshow) {
            this.sortlist = [{ id: 1, title: '销量', status: 0 }, { id: 3, title: '价格', status: 0 }]
        }
    },
    onShow() { },
    methods: {
        toCommerce(value) {
            uni.$emit('drxdrelease', { citeId: value.id, citename: value.relatedName, commissionPrice: 0 })
            uni.navigateBack()
        },
        isPreview() {
            if (this.preview === '1') {
        ...
        async setAdd(value) {
            this.isPreview()
            const { code } = await this.$request.post('/coc-active/api/v1/expert/shop/selection/add', { relatedId: value.relatedId, type: value.type })
            if (code === 200) {
        ...
        async setDelete(value) {
            this.isPreview()
            const { code } = await this.$request.post('/coc-active/api/v1/expert/shop/selection/remove', { relatedId: value.relatedId, type: value.type })
            if (code === 200) {
                this.$refs.uSquareToast.show({
                    message: '移出小店成功',
                    type: 'success'
        ...
        setList(value) {
            ...
            if (value.type === 1) {
                uni.navigateTo({
                    url: '/pages/home/activityDetail/index?id=' + value.relatedId + '&iscommission=' + iscommission
                })
            } else if (value.type === 2) {
                uni.navigateTo({
                    url: '/pages/carShops/upkeep/detail?id=' + value.relatedId + '&iscommission=' + iscommission
                })
                ...
                uni.navigateTo({
                    url: '/pages/grouppurchase/detail?id=' + value.relatedId + '&iscommission=' + iscommission
                })
            } else if (value.type === 5) {
                uni.navigateTo({
                    url: '/pages/grouppurchase/packagedetail?id=' + value.relatedId + '&iscommission=' + iscommission
                })
            } else if (value.type === 6) {
				
                uni.navigateTo({
                    url: '/pages/carShops/carList/index?id=' + value.relatedId +'&type=1&shopId=' + value.shopId
                })
            }
        },
        toShop(value) {
            ...
            if (value.shopTypeId === 2) {
                uni.navigateTo({
                    url: '/pages/carShops/4Sdetail/index?id=' + value.shopId + '&iscommission=1'
                })
            } else if (value.shopTypeId === 3) {
                uni.navigateTo({
                    url: '/pages/activityMall/business/index?id=' + value.shopId + '&iscommission=1'
                })
            }
        }
    }
