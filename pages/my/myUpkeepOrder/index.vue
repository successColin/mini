<template>
	<view class="page">
		<div class="content">
			<view class="top">
				<view v-for="(item,index) in tabs" :key="index" class="tabs" @click="changetab(item.status)"
					:style="{background: item.status==searchField.status?'#D91B1B':'#FFFFFF',color: item.status==searchField.status?'#FFFFFF':'#222222'}">
					{{item.label}}
				</view>
			</view>
			<view style="height: 24rpx;"></view>
			<view class="list" v-if="dataList.length>0">
				<view class="list_item" v-for="item in dataList" @click="OnDetail(item)">
					<view class="list_item_top">
						<view class="flex alc jsb" style="width: 100%;">
							<text class="list_item_top_orderno">订单编号:{{item.orderNo}}</text>
							<text class="list_item_top_status">{{getStatusType(item.status,item.isOverdue)}}</text>
						</view>
					...
					<view class="list_item_top_bottom">
						<view class="flex alc" style="margin-top: 24rpx;">
							...
							<view class="moreLine list_item_top_bottom_title" style="height: 75rpx;margin-left: 14rpx;">
								{{item.maintainName}}
							</view>
						</view>
						<view class="flex alc jsb" style="margin-top: 18rpx;">
							<text class="list_item_top_bottom_shop">门店：{{item.baseName}}</text>
							<text class="list_item_top_bottom_price">￥{{item.price}}X{{item.maintainNum}}</text>
						</view>
						<view class="flex alc jsb" style="height: 40rpx;margin-top: 5rpx;">
							<text class="list_item_top_bottom_time">下单日期：{{item.orderTime}}</text>
							<view v-if="item.status===0" class="list_item_top_bottom_btn1">去付款</view>
						</view>
						<view class="flex alc jsb" style="height: 40rpx;margin-top: 12rpx;">
							<text v-if="item.status==1||item.status==2||item.status==3" class="list_item_top_bottom_time">有效日期：{{item.validityDay}}</text>
						<text v-else class="list_item_top_bottom_time"></text>
							<view v-if="item.status===0" class="list_item_top_bottom_btn2" 
								@click.stop="openCancel(item)">取消</view>
							<view v-if="item.status===1" class="list_item_top_bottom_btn1"
			...
			<view v-if="dataList.length==0&&isReachBottom" style="text-align: center;padding-top: 303rpx;">
				<view>
		...
		<u-popup :show="cancelVisble" mode="center" :safeAreaInsetBottom="false" round="16rpx">
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				this.$newrequest.post("/coc-active/api/v2/maintain_meal/orderList", msg)
					.then(res => {
						if (res.code == 200) {
							if (msg.current === 1) {
			...
			//取消订单
			submitCancel() {
				this.$newrequest.get("/coc-active/api/v2/maintain_meal/cancelOrder/" + this.cancelOrderId)
					.then(res => {
						if (res.code == 200) {
							let index = this.dataList.findIndex(m => m.orderId === this.cancelOrderId)
							if (index >= 0) {
								uni.showToast({
									title: '取消成功'
								})
