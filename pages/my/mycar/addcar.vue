			...
			<u-navbar :autoBack="true">
				<view class="u-nav-slot u-flex" slot="center">
					<view style="margin-left: 100rpx;margin-right: 20rpx;">
						<u-search  placeholder="请输入品牌" v-model="keyword" @search='getList'
							@custom='getList'>
						</u-search>
					</view>
						...
						<view class="u-flex">
							<view>
								<image
									:src="item.listImg"
									mode="heightFix"
									style="width: 120rpx;height: 60rpx;"></image>
							</view>
...
<script>
	export default {
		...
		methods: {
			Onchexing(item) {
				// uni.navigateTo({
				// 	url: "/pages/my/mycar/chexing?vehicleId=" + item.vehicleId+'&vehicleName='+item.vehicleName
				// });
				uni.navigateTo({					
			...
			OnPushCar(item) {
				...
				this.brandCarLogo = item.brandCarLogo
				this.getvehicle(item.brandId)
			},
			getList() {
				this.$request.post("/coc-system/api/v1/system/brand/list", {
					...
					if(this.brandId){
						res.data.brands.filter(s=>{
							s.filter(i=>{
								if(i.brandId==this.brandId){
									console.log(i)
									this.OnPushCar(i)
								}
							})
