	...
	<view class="page">
		<view class="page-view">
			...
			<view @click="Oncertification" class="card">
				...
				<view class="u-flex-al">
					<view v-if="dataList.isCertify==1" class="card-label" @click="OnPushIdcard">
						已实名
					</view>
					<view v-else-if="dataList.isCertify==0" class="card-label" @click="OnOpendauthentication">
						未实名
					</view>
					<u-icon size="9" name="arrow-right" color="#999999"></u-icon>
				</view>
			...
			<view @click="showmodal=true" class="card" style="padding: 26rpx 0 !important;">
				<view class="title">
					<view>账号注销</view>
					<view style="font-size: 24rpx;color: #999999;font-weight: 500;">注销后无法恢复，请谨慎操作</view>
				</view>
				<view class="u-flex-al">
					<view class="card-label">
		...
		<u-popup round='10rpx' bgColor='#FFFFFF' :show="show" @close="close" mode="center" :safeAreaInsetBottom="false">
			<view class="dia-card">
				<view class="dia-title">{{dianame}}</view>
				<view style="margin-top: 130rpx;">
					<u--input v-model="newnickname" placeholder="请设置昵称" border="bottom" fontSize="28rpx"
						:adjustPosition="false"
						placeholderStyle="font-size: 28rpx;font-family: PingFang SC;font-weight: 500;color: #999999;"
						@focus="focusText" @blur="blurText">
					</u--input>
				</view>
				<view class="u-flex-al" style="justify-content: center;margin-top: 87rpx;">
					<view class="Totrue" style="margin-right: 72rpx;background: #999999;border: 1px solid #999999;"
						@click="close">
						<text>取消</text>
					</view>
		...
		<u-modal :show="showmodal" title="您确定要注销当前账号" :showCancelButton='true' @close='showmodal=false'
			@cancel='showmodal=false' @confirm='Onusercancel'></u-modal>
			<view>
...
<script>
	...
	export default {
		...
		methods: {
			OnPushIdcard(){
				uni.navigateTo({
					url: '/pages/my/info/idcardindex?name='+this.dataList.realName+'&idCard='+this.dataList.idCard
				});
			},
			...
			OnTrue() {
				let _this = this
				this.$request.post('/coc-my/api/v1/my/user/update', {
			...
			onWebView(type) {
				let url = type == 1 ? 'https://h5.dcqcjlb.com/protocol/user_privacy.html' : type == 2 ?
