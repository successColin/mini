<template>
	<view class="page">
		<view class="tabs">
			<view :class="{'current-tab-left':type==1}" @click="OnClickTab(1)" class="tab-item">收藏活动</view>
			<view :class="{'current-tab-right':type==3}" @click="OnClickTab(3)" class="tab-item">收藏团购</view>
		</view>
		<view v-if="type==1">
			<view v-if="dataList.length > 0">
				<view class="card" v-for="(item, index) in dataList" :key="index" @click="OnPushDetail(item)">
					<view class="u-flex">
						<view>
							<u-avatar shape="square" :src="item.coverPicture">
							</u-avatar>
						</view>
						<view style="margin-left: 20rpx;">
							...
							<view class="u-flex from">
								...
								<view class="fromTip" v-if="item.shopTypeId == 4">
									<image
										style="width: 104rpx;height: 28rpx;margin-left: 20rpx;margin-top: 6rpx;margin-top:8rpx"
										mode="heightFix" src="@/static/image/rendarenzheng.png" />
								</view>
								<view class="fromTip" v-else-if="item.shopTypeId == 1">
									<image src="@/static/image/guan.png" style="width: 29rpx; height:29rpx"></image>
									<view style="font-size: 24rpx;margin-top: -4rpx;color: #D91B1B;margin-left: 20rpx;">
										官方
									</view>
								</view>
					...
					<view style="10" class="rightBox">
						<view class="price">
							<view v-if="item.payType == 2">¥{{ item.flashSalePrice }}/人</view>
							<view v-else>免费</view>
						</view>
		...
		<view v-else>
			<view v-if="dataList.length > 0">
				<view class="card" v-for="(item, index) in dataList" :key="index" @click="OnPushtuangou(item)">
					<view class="u-flex">
						<view>
							<u-avatar shape="square" :src="item.coverPicture">
							</u-avatar>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getList() {
				this.$request.post('/coc-my/api/v1/my/user/collections/activity/list', {
