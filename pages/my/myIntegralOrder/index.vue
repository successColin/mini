<template>
	<view class="page">
		<div class="content">
			<view class="top">
				<view v-for="(item,index) in tabs" :key="index" class="tabs" @click="changetab(item.value)"
					:style="{background: item.value==searchField.orderStatus?'#D91B1B':'#F8F8F8',color: item.value==searchField.orderStatus?'#FFFFFF':'#222222'}">
					{{item.label}}
				</view>
			</view>
			<view v-if="tabs.length>0" style="height: 30rpx;"></view>
			<scroll-view class="bottom" scroll-y="true" @scrolltolower="onReachScollBottom"
				:style="{maxHeight:height,height:dataList.length>0?'100%':height}">
				<view class="list" v-if="dataList.length>0">
					<view class="list_item" v-for="item in dataList" @click="OnDetail(item)">
						<view class="list_item_left">
							<image class="list_item_left_img" :src="item.scoreProductTitleImg">
							</image>
						</view>
						<view class="list_item_right">
							<view class="list_item_right_top">
								<view style="display: flex;align-items: flex-start;">
									<text class="list_item_right_top_title">{{item.scoreProductName}}</text>
								</view>
							...
							<view class="list_item_right_middle">
								<text class="list_item_right_top_score">{{item.scorePrice}}</text>
								<text class="list_item_right_top_score_suffix">积分</text>
							</view>
							<view class="list_item_right_bottom">
								<text class="list_item_right_top_time">兑换时间:{{item.exchangeTime}}</text>
...
<script>
	...
	export default {
		...
		computed: {
			height() {
				return `calc(100vh - ${this.tabs.length>0?80:20}rpx - ${this.tabs.length>0?30:0}rpx)`
			}
		},
		onLoad(option) {
			//#ifdef H5
		...
		methods: {
			...
			getList() {
				...
				this.$request.post("/coc-active/api/v2/score/shop/score/order/pageList", msg).then(res => {
