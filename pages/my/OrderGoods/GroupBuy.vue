            ...
            <view class="item-right">
              ...
              >
                ...
                <view
                  ...
                  v-if="item.status == 0 && !item.iscountdown"
                >
                  <u-count-down
                    @finish="setcountdown(index)"
...
export default {
  ...
  methods: {
    confirm() {
      if (this.orderType == 5) {
        this.$request
          .post('/coc-active/api/v1/merchant_meal_order/cancel', {
            id: this.signUpId,
          })
          .then((res) => {
            if (res.code == 200) {
        ...
        this.$request
          .get('/coc-active/api/v1/group_purchase/cancelOrder/' + this.signUpId)
          .then((res) => {
            if (res.code == 200) {
              uni.showToast({
                title: '取消订单成功',
    ...
    getList() {
      this.$request
        .post('/coc-active/api/v1/group_purchase/orderTabulation', {
          size: 20,
          current: this.current,
          status: this.status,
          orderType: this.typeNum
        })
        .then((res) => {
