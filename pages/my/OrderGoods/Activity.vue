  ...
  <view class="page">
    ...
    <view v-if="type == 4" style="display: flex">
      <view
        class="evaluatebtn"
        :class="[
          seacher.isComments == 1 ? 'evaluatebtn-color2' : 'evaluatebtn-color1',
        ]"
        @click="clickisComment(1)"
        >待评论</view
      >
      <view
        class="evaluatebtn"
        :class="[
          seacher.isComments == 2 ? 'evaluatebtn-color2' : 'evaluatebtn-color1',
        ]"
        @click="clickisComment(2)"
        >已评论</view
      >
    </view>
              ...
              <view>
                ...
                <view class="order-detail">
                  <view style="font-size: 28rpx">
                    <text v-if="item.activityPayType == 2"
                      >¥{{ item.paidMoney }}</text
                    >
                    <text v-else>免费</text>
                  </view>
            ...
            <view class="item-right">
              ...
              >
                ...
                <view v-if="type == 4" class="payment-four">
                  <text @click.stop="OnDetail(item)">{{
                    seacher.isComments == 1
                      ? '去评论'
                      : seacher.isComments == 2
                      ? '已评论'
                      : ''
                  }}</text>
                </view>
                ...
                <view v-else-if="item.status === 1" class="type-2">
                  ...
                  <view
                    v-if="item.activityPayType == 2 && item.nonRefund == 0"
                    class="refund"
                  >
...
export default {
  ...
  methods: {
        ...
        success: function (res) {
          uni.showToast({
            ...
            complete() {
              ...
              uni.navigateTo({
                url:
                  '/pages/my/myorder/orderList?orderNo=' +
                  _this.dataList.activitySignUp.orderNo,
              });
            },
          });
        },
    ...
    confirm() {
      this.$request
        .post('/coc-active/api/v1/sign_up/cancel', {
    ...
    OnDetail(item) {
      if (this.type == 4) {
        uni.navigateTo({
          url:
            '/pages/home/wonderfulReview/detail?id=' +
            item.activityMarvellousId,
        });
      } else {
        uni.navigateTo({
    ...
    getList() {
      ...
      if (this.typeNum) {
        msg.shopCategoryId = this.typeNum;
      }
      if (this.shopTypeNum) {
        msg.shopTypeId = this.shopTypeNum;
      }
    ...
    Onrefund(item) {
      uni.navigateTo({
        url:
          '/pages/my/myorder/refund?orderNo=' +
          item.orderNo +
          '&orderCode=' +
          item.orderCode +
          '&orderMoney=' +
