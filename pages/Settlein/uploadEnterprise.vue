	...
	<view class="page">
		...
		<view class="card" v-if="qualificationsList.length>0">
			...
			<view v-for="(item,index) in qualificationsList" :key="index" @click="Onspecialimg(index)">
				...
				<view v-else class="Indoor-special"
					:style="{ backgroundImage: 'url(' + qualificationsImg[index] + ')', }">
				</view>
			</view>
		</view>
		...
		<u-popup :safeAreaInsetBottom='false' :show="mccshow" @close="mccshow=false" mode="center">
			<view class="pop-mcc">
				<view class="tags">
					<view :class="{'current-tags':index==currentmcc}" class="tags-type"
						v-for="(item,index) in industrylist" :key="item" @click="currentmcc=index">{{item.typeMsg}}
					</view>
				</view>
				<view style="margin-top: 40rpx 20rpx 20rpx 20rpx;">
					<view class="scc-value mccitem" v-for="(item1,index1) in industrylist[currentmcc].industryVOS"
						:key="index1" @click="Onchoosemcc(item1)">
		...
		methods: {
			...
			getallList() {
				this.$request.post("/coc-active/api/v1/merchantsettledin/dataEcho", {
					...
					// this.dataList.picList = res.data.auditFailed.shopPiclistDTOS
			...
			getList() {
				this.$request.post("/coc-active/api/v1/merchantsettledin/dataEcho", {
					...
					this.dataList.DoorheadImg = res.data.settlementMaterials.doorheadPhoto
			...
			Submit() {
				...
				if (this.dataList.juridicalIdCardFrontImg && this.dataList.juridicalIdCardBackImg && this.dataList
					.licenseImg && this.dataList.DoorheadImg && this.dataList.IndoorImg&&this.dataList.indoorPhotograph2 &&
					...
					uni.navigateTo({
						url: '/pages/Settlein/enterpriseInfo',
