<template>
	<view class="page">
		<u--form labelPosition="left" :model="dataList" :rules="rules" ref="form" labelWidth='220rpx'
			:labelStyle="{fontSize:'24rpx'}">
			<view class="card">
				<u-form-item label="入驻商家名称" prop="shopName" :required='true' borderBottom>
					<u--input :disabled='!isshow' inputAlign='right' border="none" placeholder="请输入名称" v-model="dataList.shopName"
						:customStyle="{backgroundColor:'#ffffff'}">
					</u--input>
				</u-form-item>
				<u-form-item label="法定代表人姓名" prop="legalName" :required='true' borderBottom>
					<u--input :disabled='!isshow' inputAlign='right' border="none" placeholder="请输入姓名" v-model="dataList.legalName"
						:customStyle="{backgroundColor:'#ffffff'}">
					</u--input>
				</u-form-item>
				<u-form-item label="法定代表人手机号" prop="legalMobile" :required='true' borderBottom>
					<u--input :disabled='!isshow' inputAlign='right' border="none" placeholder="请输入手机号" v-model="dataList.legalMobile"
						:customStyle="{backgroundColor:'#ffffff'}">
					</u--input>
				</u-form-item>
				<u-form-item label="法定代表人邮箱" prop="legalEmail" :required='true'>
					<u--input :disabled='!isshow' inputAlign='right' border="none" placeholder="请输入法人邮箱" v-model="dataList.legalEmail"
						:customStyle="{backgroundColor:'#ffffff'}">
					</u--input>
				</u-form-item>
			</view>
			<view class="card">
				<u-form-item label="身份证号" prop="legalIdcardNo" :required='true' borderBottom>
					<u--input :disabled='!isshow' inputAlign='right' border="none" placeholder="请输入身份证号" v-model="dataList.legalIdcardNo"
						:customStyle="{backgroundColor:'#ffffff'}">
					</u--input>
				</u-form-item>
				<u-form-item label="证件截止日期" prop="legalCardDeadline" :required='true' borderBottom>
					<view @click="Onshowtime">
						<u--input :disabled='!isshow' inputAlign='right' border="none"  placeholder="请选择截至日期"
							v-model="dataList.legalCardDeadline" :customStyle="{backgroundColor:'#ffffff'}">
						</u--input>
					</view>
				</u-form-item>
				<u-form-item label="家庭地址" prop="legalmanHomeAddr" :required='true'>
					<u--input v-if="isshow" inputAlign='right' border="none" placeholder="请输入法人家庭地址"
						v-model="dataList.legalmanHomeAddr" :customStyle="{backgroundColor:'#ffffff'}">
					</u--input>
					<view v-else>
						{{dataList.legalmanHomeAddr}}
					</view>
				</u-form-item>
			</view>
			<view class="card">
				<u-form-item label="企业注册地" prop="areatext" :required='true' borderBottom>
					<view @click="OnAreaCode">
						<u--input inputAlign='right' border="none" :disabled='true' placeholder="请选择企业注册地"
							v-model="dataList.areatext" :customStyle="{backgroundColor:'#ffffff'}">
						</u--input>
					</view>
				</u-form-item>
				<u-form-item label="营业地址" prop="shopAddrExt" :required='true' borderBottom>
					<u--input v-if="isshow" inputAlign='right' border="none" placeholder="请输入公司或营业门店地址" v-model="dataList.shopAddrExt"
						:customStyle="{backgroundColor:'#ffffff'}">
					</u--input>
					<view v-else>
						{{dataList.shopAddrExt}}
					</view>
				</u-form-item>
				<u-form-item label="统一社会信用代码" prop="shopLic" :required='true'>
					<u--input :disabled='!isshow' inputAlign='right' border="none" placeholder="请输入统一社会信用代码" v-model="dataList.shopLic"
						:customStyle="{backgroundColor:'#ffffff'}">
					</u--input>
				</u-form-item>
			</view>
			...
			<view class="card">
				<u-form-item v-if="isshow" label="开户行所在地" :required='true' borderBottom>
					<view @click="OnBankAreaCode">
						<u--input inputAlign='right' border="none" :disabled='true' placeholder="请选择开户行所在地"
							v-model="dataList.areacity" :customStyle="{backgroundColor:'#ffffff'}">
						</u--input>
					</view>
				</u-form-item>

				<u-form-item label="开户行" v-if="dataList.areacity||!isshow" prop="bankName" :required='true'
					borderBottom>
					<u-search v-if="isshow" :disabled='!isshow' @search='seachBank' @custom='seachBank' searchIconSize='0' searchIconColor='#ffffff'
						bgColor="#ffffff" placeholder="请选择开户行" v-model="dataList.bankName" :showAction="true"
						actionText="搜索" :animation="false"></u-search>
						<view v-else>
							{{dataList.bankName}}
						</view>
				</u-form-item>
				<u-form-item label="开户账号" prop="bankAcctNo" :required='true' borderBottom>
					<u--input :disabled='!isshow' inputAlign='right' border="none" placeholder="请输入银行开户账号" v-model="dataList.bankAcctNo"
						:customStyle="{backgroundColor:'#ffffff'}">
					</u--input>
				</u-form-item>
				<u-form-item label="开户名称" prop="bankAcctName" :required='true'>
					<u--input :disabled='!isshow' inputAlign='right' border="none" placeholder="请输入银行开户名称" v-model="dataList.bankAcctName"
						:customStyle="{backgroundColor:'#ffffff'}">
					</u--input>
				</u-form-item>
			</view>
		...
		<view v-if="isshow" style="font-size: 24rpx;margin: 40rpx 0rpx;" class="u-flex">
			...
			<view>我确认以上信息并同意签署<text style="color: #d91b1b;" @click="OnPushCooperation">《商家合作协议》</text></view>
		</view>
		<view v-if="isshow" @click="OnSubmit" class="OnSubmit">
			下一步
		</view>
		<!--  #ifdef MP-WEIXIN  -->
		...
		<!--  #endif -->
		<u-datetime-picker ref="datetimePicker" :show="showtime" mode="date" :minDate="today" :maxDate='2556098656000'
			@confirm='Ontimedate' @cancel='showtime=false'>
		</u-datetime-picker>
		<u-picker @cancel="showBankareaCode=false" :show="showBankareaCode" ref="uPicker1" :columns="columnsBank"
			@change="changeBankAreaCode" @confirm="confirmBankareaCode" keyName="label"></u-picker>
		...
		<u-popup :safeAreaInsetBottom='false' :show="popbank" @close="popbank=false" mode="center">
			<view class="pop">
				<u-search @search='seachBankList' @custom='seachBankList' searchIconSize='0' searchIconColor='#ffffff'
					bgColor="#ffffff" placeholder="请选择开户行" v-model="dataList.bankName" :showAction="true"
					actionText="搜索" :animation="false"></u-search>
				<scroll-view style="height: 500rpx;margin-top: 40rpx;" :scroll-top="scrollTop" scroll-y="true"
					class="scroll-Y">
					<view v-for='(item,index) in bankList' :key="index" class="list" @click="OnChoose(item,index)">
						...
						<view>
							<image v-if="chooseBank!=index" style="width: 30rpx;height: 30rpx;"
								src="@/pages/Settlein/static/image/weixuanzhong.png"></image>
							<image v-else style="width: 30rpx;height: 30rpx;"
			...
			<view class="pop-agreement">
				...
				<view style="margin-top: 40rpx;margin-bottom: 40rpx;">
					<scroll-view scroll-y="true" style="height: 400rpx;text-align: left;font-size: 20rpx;">
						<text style="font-size: 24rpx;">
							...
							2.3.乙方在甲方平台入驻开店成功后，应当根据法律、法规及平台规则发布商品、提供售前售后服务及其他各项服务，双方另有约定或甲方平台规则另有规定的除外；同时乙方应依法为消费者开具正规的商品发票，相关税费由乙方自行承担。
							2.4.甲方服务范围。
							2.4.1.甲方负责大昌汽车俱乐部平台的日常维护、技术支持，保证平台的正常运作；作为电子商务平台经营者，甲方仅为乙方与消费者达成交易提供网页空间、虚拟经营场所、交易规则等服务，甲方并非乙方与消费者之间交易行为的参与方，不对乙方及/或消费者的任何口头、书面陈述或承诺，发布的信息及交易行为的真实性、合法性、准确性、及时性、有效性等作任何明示或暗示的保证。
							2.4.2调整商品展示位置。除双方另有约定外，甲方有权根据商品销量、商品价格、商品质量、乙方的服务水平等综合因素调整商品的展示位置。
							 
							3.甲方有限责任
				...
				<view class="pop-bottom" @click="Onqianshu">
					...
					<view v-if="!isfinish" style="display: inline-block;">
						<u-count-down @finish='Onfinish' :autoStart="false" ref="countDown" :time="10*1000" format="ss"
							@change="onChange">
	...
	export default {
		...
		methods: {
			...
			seachBankList() {
				...
				this.$request.post("/coc-active/api/v1/merchantsettledin/subBranchQuery", {
			...
			Ontimedate(val) {
				...
				var mm = date.getMinutes() < 10 ? "0" + date.getMinutes() : date
					.getMinutes();
				this.dataList.legalCardDeadline = date.getFullYear() + "-" +
					month +
					"-" +
					currentDate
				this.showtime = false
			...
			OnSubmit() {
				...
				this.$refs.form.validate().then(res => {
					uni.navigateTo({
						url: '/pages/Settlein/uploadAccountOpening',
