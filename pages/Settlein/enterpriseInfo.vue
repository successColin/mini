    ...
    <view class="page">
        ...
        >
            <view class="card">
                ...
                <u-form-item label="法定代表人邮箱" prop="legalEmail" :required="true" :borderBottom="identity == 2">
                    <u--input
                        :disabled="!isshow"
                        inputAlign="right"
                        border="none"
                ...
                <view style="margin-top: 40rpx; margin-bottom: 40rpx">
                    <scroll-view scroll-y="true" style="height: 400rpx; text-align: left; font-size: 20rpx">
                        <text style="font-size: 24rpx">
                            ...
                            5.16.乙方不得有下列行为： 5.16.1.额外向用户收取除大昌汽车俱乐部平台明示以外的其他费用；
                            5.16.2.以收取加盟费等形式开发下线。 5.17.乙方对甲方引流的用户积极响应，促进交易生成。 5.18.
                            ...
                            （6）甲方自主决定解除本协议，并以书面形式提前通知乙方； （7）可以解除协议的其他情形。
                            13.3.协议终止后事项的处理
...
export default {
    ...
    methods: {
        ...
        getList() {
            this.$request
                ...
                .then((res) => {
                    ...
                    this.dataList.legalCardDeadline = res.data.messageConfirmations.juridicalCardDeadline?.slice(0, 10);
                    ...
                    this.identity =
                        res.data.messageConfirmations.agentMobile === "" ||
                        res.data.messageConfirmations.agentMobile === null
                            ? 1
                            : 2;
                });
        },
        ...
        OnSubmit() {
                                            ...
                                            .then((res) => {
                                                this.$refs.uToast.show({
                                                    ...
                                                    complete() {
                                                        uni.navigateTo({
                                                            url:
                                                                "/pages/Settlein/enterpriseProgress?type=1&isLineLedger=" +
                                    ...
                                    if (res.code == 200) {
                                        this.$refs.uToast.show({
                                            ...
                                            complete() {
                                                ...
                                                uni.navigateTo({
                                                    url:
                                                        "/pages/Settlein/enterpriseProgress?type=2&isLineLedger=" +
                                                        _this.dataList.isLineLedger +
                                                        "&settleType=" +
                                                        _this.settleType,
                                                });
            ...
            // 复制下一步的方法 主要上传开户证明（线下企业）
            // if (this.dataList.isLineLedger == 1) {
            //   if (this.checkboxValue1.length == 0) {
            //     uni.$u.toast('请确认阅读并同意商务隐私政策')
            //     return false
            //   }
            // }
            // if (this.dataList.isLineLedger == 1) {
            ...
            //   if (res.document_type == '0006') {
            //     res.document_name = '开户许可证（企业商户必传)'
            //   } else if (res.document_type == '0007') {
            //     res.document_name = '手持身份证自拍照'
            //   }
            // })
            // this.$request.post("/coc-active/api/v1/merchantsettledin/dataUpload",
            //   this.dataList
            // ).then(res => {
            //   if (res.code == 200) {
            //     this.corporatePayment(res.data)
            //   } else {
            //     this.$refs.uToast.show({
            //       message: res.message,
            //     })
